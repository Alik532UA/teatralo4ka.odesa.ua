...
- **Розділ на сторінці — ПОХІДНА (`masterSection` у `data/masters.ts`)**, не поле.
  Порядок умов значущий: `honorary` → повнота → `former` → роль. Повнота = **є
  фотографія І повне ПІБ** (посади в умові немає навмисно: її не має жоден із
  трьох «Світлої пам'яті» й шість із семи «Історії школи» — умова з нею
  спорожнила б обидва розділи). Склад розділів після переходу на похідну:
  `administration` 5, `heads` 3, `pedagogues` 13, `production` 2, `it` 1,
  `support` 7, `honorary` 3, `history` 8, `needsClarification` 76.
  Проти ручного поділу розходження в **девʼяти записах**, і кожне на користь
  правила: чотири поїхали з «Історії школи» в технічний розділ (Ткач Ганна — без
  фотографії; Шевчук Олег В., Рибакова Надія В. — ініціал замість по батькові;
  Стельмах — лише прізвище), пʼять навпаки (Імас Ольга, Домбровська, Мазур,
  Пруба, Серебряннікова Наталія — у всіх є і фото, і повне ПІБ). Старе ручне
  правило вже не діяло: фотографію мали **36 із 77** записів того розділу.
...
- **Порядок сортування в категоріях:**
  - `administration`: фіксована послідовність (1. Олена Ткач, 2. Оксана Панченко, 3. Наталія Шалашна, 4. Любов Франковська, 5. Тетяна Коренчук — директорка, заступниця, далі бухгалтерія). Лілія Величко прибрана 2026-08-24 (переїхала в «Історію школи», запис тут був мертвий), тоді ж троє завідувачок поїхали у власний розділ `heads`, тож із цього переліку їх теж прибрано: залишений запис так само нічого не робив би, бо сортування бачить лише записи свого розділу.
  - `heads`: фіксовано 1. Світлана Риськіна (театральне), 2. Віра Коваль (художнє), 3. Ганна Ніколаєва (музичне) — порядок успадкований із того, як троє стояли в керівництві до розділення. Без переліку порядок визначала б кількість учнів, і Риськіна з 37 учнями завжди була б першою, а решта двоє перескакували б місцями від кожної правки в даних випускників.
  - `pedagogues` та `history`: **за кількістю учнів за спаданням, далі «з фотографією вперед», далі алфавіт за `fullName`.** Поля `graduatesCount` у `masters.index.json` справді немає ні в кого — його додає `+page.ts` із `getGraduatesByMaster`, і воно ЖИВЕ: заміряно 2026-08-24, учні є у **26 зі 118** майстрів, від 1 до 37 (Риськіна 37, Ісачкіна 27, Ткач Ф. 16). До 2026-08-24 тут і в коментарі коду стояло, що це порівняння завжди дає нуль і нічого не додає — неправда, яка описувала сортування, якого немає.
  - `needsClarification`: алфавіт за `fullName` (`localeCompare(…, 'uk')`), і тільки він. Без кількості учнів і без «з фотографією вперед»: у цей розділ запис попадає саме за брак фотографії або повного імені, тож обидві ознаки тут майже однакові для всіх.
