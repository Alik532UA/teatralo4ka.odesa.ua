...
- **ТРИ ОСІ, а не одне поле** (розділено 2026-08-24; до того все це жило в `category`):
  - `category` — **функціональна роль**: `administration` (8), `pedagogues` (23),
    `production` (2), `it` (1), `support` (7). Поля **немає у 77 записів** — роль
...
- **Розділ на сторінці — ПОХІДНА (`masterSection` у `data/masters.ts`)**, не поле.
  ...
  розділів: `administration` 5, `heads` 3, `pedagogues` 13, `production` 1, `it` 1,
  `support` 7, `honorary` 3, `history` 18, `needsClarification` 67.
...
- **«Роки роботи» — `periods` у ПРОФІЛІ, не в індексі.** `{ from: number; to: number | null }[]`,
  ...
  118 людей везлися б на кожну сторінку заради одного рядка на сторінці профілю
