...
describe('TokenResolver', () => {
	...
	it('тема переважує global.css, а не навпаки', () => {
		// --bg-card у світлій темі білий, у темній — graphite-800 (#222831).
		//
		// Число тут мінялося двічі, і обидва рази з тієї самої причини: автор
		// дивився основну темну поруч із іншою й просив помінятися кольорами.
		// 2026-09-08 navy-page поступилася steel-900 (#1B262C, обмін із
		// `dev-test-dark-01`), 2026-09-14 steel-900 — графіту (обмін із
		// `dark-cyan`, куди сталева й поїхала).
		//
		// Саме тому перевірка й цінна: вона не про конкретний відтінок, а про
		// те, що тему бере ТЕМА, а не `global.css`. Обмін палітр щоразу
		// проходить через цей рядок, і мовчки він статися не може.
		expect(r.resolve('--bg-card', 'light')).toEqual([255, 255, 255]);
		expect(r.resolve('--bg-card', 'dark')).toEqual([34, 40, 49]);
	});
