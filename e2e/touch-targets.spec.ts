...
test.describe('цілі дотику', () => {
	for (const path of PAGES) {
		test(`${path}`, async ({ page }, testInfo) => {
			...
			/*
			 * Перевірка повільна за побудовою, і межу часу піднято НАВМИСНО.
			 *
			 * Кожна ціль міряється окремим зверненням до сторінки — видимість,
			 * `display`, бокс. Інакше не можна: `isVisible()` у Playwright знає
			 * про `opacity` й перекриття те, чого просте читання DOM не знає, а
			 * саме через це підвал колись і давав різні набори між прогонами.
			 *
			 * Ціна — час. Заміряно 2026-08-31: окремо цей файл проходить за
			 * 15,6 с при двох робітниках, тобто половину типової межі. У повному
			 * наборі, де робітників більше й вони конкурують за процесор, той
			 * самий прохід перевалював за 30 с — і падав за часом, нічого не
			 * стверджуючи про самі цілі. `test.slow()` потроює межу; це чесніше,
			 * ніж прискорювати перевірку послабленням того, що вона міряє.
			 */
			test.slow();
