...
test.describe('пошук по сайту', () => {
	...
	test('роль з анкети знаходить і виставу, і людину', async ({ page }) => {
		...
		await expect(page.locator('.search__hit').first()).toBeVisible({ timeout: 10_000 });
		const усі = (await page.locator('.search__hit').allInnerTexts()).join(' | ');
		expect(усі, 'вистави з цим номером немає серед знайденого').toMatch(
			/Уривки з драматургії/
		);
		expect(усі, 'людини з цією роллю немає серед знайденого').toMatch(/Запольнов/);
	});
