...
/**
 * Тотожність сервера тут БІЛЬШЕ НЕ ПЕРЕВІРЯЄТЬСЯ — вона переїхала в
 * `identity.setup.ts`, сетап-проєкт, від якого залежать обидва браузерні
 * (CI-CD-AND-TOOLS-v9, `CI-E2E-TARGET-IDENTITY`).
 *
 * Причина переїзду, а не копії: цей тест був написаний першим у файлі й саме
 * тому вважався першим у прогоні. При `fullyParallel: true` це неправда —
 * решта файлів іде одночасно з ним, тобто на чужому сервері встигали
 * відпрацювати сотні перевірок. Сетап-проєкт зупиняє прогін ДО них, і заразом
 * звіряє штамп збірки, чого тут не було зовсім.
 */
...
test('sitemap перелічує тільки адреси, які справді існують', async ({ page }) => {
	// 2706 адрес по HTTP не вкладаються в типові 30 секунд — див. пояснення нижче.
	 ...
	 * саму властивість, заради якої існує. Пакет лишає перевірку тією самою:
	 * жодна адреса не пропущена, просто чекання йде паралельно.
	 *
	 * 2026-09-01: адрес стало 2706 — власну сторінку отримали всі 530
	 * випускників, а не 93 з анкетами. Пакет по 25 давав 109 послідовних кроків
	 * і знову не вкладався у 30 с: тест упав за часом у chromium і в mobile, хоч
	 * сама мапа була ЦІЛА (перевірено окремо: усі 2706 адрес мають файл у
	 * `build/`). Тому власна стеля часу.
	 *
	 * Пакет лишається 25, хоч спокуса була збільшити. Заміряно: з пакетом 75
	 * тест проходив, зате валив ТРИ сусідні — `/en/departments/art`,
	 * `/en/departments/aesthetic`, `/en/residents/kids` падали з
	 * `net::ERR_INSUFFICIENT_RESOURCES`. Тести йдуть паралельно, і сімдесят
	 * п'ять одночасних запитів з'їдали сокети в усіх. Тобто вузьке місце тут не
	 * швидкість обходу, а СТЕЛЯ ЧАСУ, і правити треба було саме її.
	 *
