...
test.describe('перелік вистав', () => {
	...
	/**
	 * НАБРАТИ Й ДОЧЕКАТИСЯ РЕАКЦІЇ, а не набрати й одразу читати.
	 *
	 * Сторінка пререндерена: розмітка приходить готовою, а обробники з'являються
	 * лише після гідратації. Текст, набраний до неї, лягає в DOM, але жоден
	 * слухач його не бачить — і фільтр не спрацьовує ВЗАГАЛІ, скільки не чекай.
	 * Виглядає це як «пошук не звузив нічого»: у переліку всі 273 вистави й
	 * жодного лічильника.
	 *
	 * Ловилося це на мобільному проєкті приблизно раз на прогін і стало
	 * постійним 7 вересня 2026, коли в переліку з'явився `+page.ts` — сторінка
	 * тепер чекає ще й на свій `__data.json`, тобто гідратується пізніше. Тому
	 * набір повторюється, поки сторінка не відповість.
	 */
	async function набрати(
		page: import('@playwright/test').Page,
		поле: import('@playwright/test').Locator,
		текст: string
	) {
		const лічильник = page.getByTestId('galaxy-plays-search-count-text');
		for (let спроба = 0; спроба < 10; спроба += 1) {
			await поле.fill(текст);
			try {
				await лічильник.waitFor({ state: 'visible', timeout: 1000 });
				return;
			} catch {
				await поле.fill('');
			}
		}
		throw new Error(`поле пошуку не відповідає на «${текст}» — сторінка не ожила`);
	}
	...
	test('пошук звужує перелік і чесно каже, коли нічого немає', async ({ page }) => {
		...
		await набрати(page, пошук, 'Цибуліно');
	...
	test('типово показані лише вистави з відомим складом, і пошук цим не обмежений', async ({
		 ...
		 */
		const рядки = page.locator('[data-testid^="galaxy-plays-row-link-"]');
		const обличчя = page.locator('[data-testid^="galaxy-plays-cast-"][data-testid$="-list"]');

		await expect(рядки.first(), 'типово не показано жодної вистави').toBeVisible();

		/*
		 * РІЗНИЦЯ міряється з повтором, а не двома знімками поспіль.
		 *
		 * Доти тут стояли два `count()` — а `count()` знімає DOM миттєво й не
		 * повторюється. На завантаженій машині перелік ще добудовувався, і два
		 * заміри припадали на РІЗНІ миті цієї добудови: рядки вже пораховані,
		 * обличчя ще ні. Тест падав на різниці, якої в готовій сторінці немає, і
		 * саме він двічі підводив до хибного висновку про причину.
		 *
		 * Сама вимога не послабилася: різниця мусить дійти до нуля, і рядок без
		 * облич не дасть їй цього зробити за жоден час.
		 */
		await expect
			.poll(async () => (await рядки.count()) - (await обличчя.count()), {
				message: 'серед показаних є вистава без жодного учасника'
			})
			.toBe(0);
