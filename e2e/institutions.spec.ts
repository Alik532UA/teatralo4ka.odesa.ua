 ...
 *
 * Троє зі сімнадцяти студентів реєстру випускників були невідомі
 * (`DATA-QUESTIONS.md` § 8.3), і сторінка ОТХФК показувала двох таких ТЕКСТОМ.
 * Це і є та перевірка, яку легко втратити: викинути незіставлених — і сторінка
 * покаже нуль людей, хоч ми знаємо двох, причому без жодної помилки в даних.
 *
 * 2026-09-05 усіх трьох зіставили — вони виявилися нашими УЧНЯМИ, — тож даних
 * для цієї перевірки в реєстрі не лишилося, і вона пропускається з причиною.
 * Сам показ текстом зі сторінки не прибраний: наступне джерело так само може
 * назвати людину, якої в реєстрі немає.
 *
...
/*
 * `id` людини в реєстрі закладу й `slug` у `data-testid` картки — це РІЗНІ
 * рядки: у частини випускників slug свій (наприклад, `deva144p`). Тому мапа, а
 * не здогад — інакше перевірка шукала б картку, якої немає.
 */
const СЛАГИ = new Map<string, string>(
	(
		JSON.parse(readFileSync('src/lib/data/graduates.index.json', 'utf8')) as ReadonlyArray<{
			id: string;
			slug: string;
		}>
	).map((g) => [g.id, g.slug])
...
test.describe('навчальні заклади', () => {
	...
	test('сторінка закладу показує всіх студентів із роком вступу', async ({ page }) => {
		 ...
		 *
		 * Перевіряється КОНКРЕТНА картка людини, яка рік має, а не перша-ліпша.
		 * Перша редакція брала `картки.first()` і 2026-09-05 упала: у реєстрі
		 * КНУТКіТ четверо з 42 без року вступу, і саме такий опинився першим —
		 * перевірка сказала «року немає в підписі», хоч підпис був правильний.
		 * Тобто вона стерегла ПОРЯДОК даних, а не рік у підписі.
		 */
		const зРоком = НАЙБІЛЬШИЙ.students.filter((s) => s.year);
		expect(
			зРоком.length,
			'у реєстрі закладу немає жодного року вступу — перевіряти нема чого'
		).toBeGreaterThan(0);
		const слаг = СЛАГИ.get(зРоком[0].id) ?? зРоком[0].id;
		await expect(
			page.getByTestId(`institution-student-card-${слаг}`),
			`у підписі студента немає року вступу (${зРоком[0].id})`
		).toContainText(String(зРоком[0].year));
	});
	...
	test('незіставлені студенти показані текстом, а не викинуті', async ({ page }) => {
		...
		/*
		 * 2026-09-05 незіставлених не лишилося — і це закрите питання, а не
		 * зламана перевірка. Усіх трьох (`DATA-QUESTIONS.md` § 8.3) зіставили:
		 * вони виявилися нашими УЧНЯМИ, тобто вже в реєстрі людей, просто з
		 * іншим різновидом. Показ текстом у сторінці лишився, і щойно джерело
		 * назве наступного невідомого — перевірка знову працює.
		 */
		test.skip(!заклад, 'у реєстрі не лишилося незіставлених студентів');
