...
const ОБХІД = `(() => {
	...
	for (const a of document.querySelectorAll('a')) {
		...
		/*
		 * РОЗТЯГНУТЕ посилання — не слово в реченні, а ціла картка.
		 *
		 * Взірець такий: сам рядок переліку позиційований, а посилання накриває
		 * його псевдоелементом \`inset: 0\` — тобто натискається весь рядок, а не
		 * назва в ньому. Помітність тут дає картка: рамка, тло й підсвітка під
		 * курсором. Підкреслити в ній назву означало б сказати «ось це слово
		 * клікабельне», хоча клікабельне все.
		 *
		 * Без цього рядка перевірка ловила переліки галактики: сторінка
		 * «Театр.PRO» перейшла на спільний рядок переліку, і назва в кожному
		 * рядку стала «непомітним посиланням» — при тому, що сам рядок
		 * відгукується на курсор усією площею.
		 */
		const перед = getComputedStyle(a, '::before');
		if (перед.position === 'absolute' && перед.inset === '0px') continue;
