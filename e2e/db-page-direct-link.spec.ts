...
test.describe('новина з бази за прямим посиланням', () => {
	test('новина з адмінки відкривається', async ({ page }) => {
		/*
		 * Власні чекання цієї перевірки — 20 + 30 + 30 секунд, тобто типових
		 * тридцяти на весь тест не вистачало НІКОЛИ: відлік з'їдався раніше за
		 * будь-яке з них, і замість зрозумілого «новин з бази нуль» у звіті
		 * стояло «Test timeout exceeded» — тобто причина ховалася саме тоді,
		 * коли вона була.
		 */
		...
		await expect
			.poll(async () => (await новиниЗБази()).length, {
				timeout: 30_000,
				message:
					'у переліку не з’явилося жодної новини з бази — перевіряти нема чого. \n' +
					'Перше, що варто звірити, — `VITE_PROJECT_ID` у робочому процесі: це ключ \n' +
					'розділу даних (`projects/<id>/articles`), і чуже значення дає порожній \n' +
					'перелік БЕЗ жодної помилки — рівно те, що видно тут.'
			})
			.toBeGreaterThan(0);
