...
test('піаніно: звук не блокується політикою', async ({ page }) => {
	/*
	 * Межу часу піднято: перевірка йде через ВСЮ головну.
	 *
	 * Послідовність тут довга — гідрація сторінки з сотнями карток, прокрутка до
	 * підвалу, чекання на кінець анімації появи, відкриття модалки, монтування
	 * `<audio>`. Поодинці це секунди, під повним прогоном — більше за типову
	 * межу в 30 с, і тест падав за часом, нічого не кажучи про політику. Те саме
	 * вже записано в `touch-targets.spec.ts`: коротшати за рахунок того, що
	 * міряється, тут не можна — саме дія користувача й відкриває ці ресурси.
	 */
	test.slow();
...
test('джерела звуку піаніно дозволені політикою', async ({ page }) => {
	test.slow(); // Та сама причина, що й у сусідній перевірці вище.
	await gotoReady(page, '/');
	const csp = await page.locator('meta[http-equiv="Content-Security-Policy"]').getAttribute('content');
