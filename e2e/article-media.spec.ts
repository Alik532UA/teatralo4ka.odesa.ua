...
test.describe('медіа новини', () => {
	...
	test('велика галерея новини нічого не губить і відкривається', async ({ page }) => {
		...
		/*
		 * Стовпець несе ПАРУ (рамка з обкладинкою й кнопка), а не плитки, — тож у
		 * галереї мають опинитися всі інші медіа.
		 *
		 * Рахуються ПЛИТКИ ОБОХ ВИДІВ, а не самі знімки, і це не педантизм: у цієї
		 * новини від 2026-09-14 два записи, і другий стоїть першою плиткою
		 * галереї. Перша редакція звіряла дітей галереї з кількістю ЗНІМКІВ і
		 * впала на 34 проти 33 — тобто звинуватила сторінку в загубленому знімку
		 * там, де насправді був зайвий для неї запис. Наступне відео зламало б її
		 * так само.
		 */
		const плиткиГалереї = page.locator(
			'[data-testid^="article-media-photo-btn-"], [data-testid^="article-media-video-btn-"]'
		);
		const решта = await page.evaluate(
			() => document.querySelector('.article-media-rest')?.children.length ?? 0
		);
		expect(решта, 'частина медіа не потрапила в галерею').toBe(await плиткиГалереї.count());
