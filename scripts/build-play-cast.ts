...
export interface CastEntry {
	...
	items?: string[];
	/**
	 * Роль у КОЖНОМУ номері окремо. Саме `role` цього не виражає: у вечорі з
	 * дев'яти уривків одна людина грає Креонта, Поета, Мокія й Миколу, і «перша
	 * названа» роль на картці приписувала б їй Креонта в кожному з чотирьох.
	 *
	 * Перелік пар, а не мапа «номер → роль». JSON-мапу, у якій різні записи
	 * мають різні ключі, TypeScript виводить як один об'єкт з усіма ключами й
	 * необов'язковими `undefined`-полями — і зріз перестає приводитися до свого
	 * ж типу (`svelte-check` падав саме на цьому). Перелік однакових об'єктів
	 * виводиться точно.
	 */
	roles?: { item: string; role: string }[];
...
export function buildCast(): Record<string, CastEntry[]> {
	...
	for (const file of files) {
		...
		for (const play of anketa.plays ?? []) {
			 ...
			 * було в анкеті.
			 *
			 * Рядків буває кілька: в «Уривках з класики» 2013 Алік Запольнов грав
			 * і в «Антігоні», і в «Незнайомці», і в «Сонетах» — три різні номери,
			 * у кожного своя роль, а роль живе в рядку. На сторінці ж це одна людина:
			 * без злиття `{#each}` діставав три записи з тим самим ключем і падав з
			 * `each_key_duplicate`, тобто сторінка показу не відкривалася зовсім.
			 *
			 * Номери об'єднуються, роль береться перша названа, а по уривках
			 * розкладається в `roles`; `fromRegistry` лишається тільки тоді, коли
			 * ЖОДЕН із рядків не є словами самої людини: досить одного власного
			 * рядка, щоб заява перестала бути «зі списку школи».
			 */
			...
			if (play.items?.length) {
				...
				/*
				 * Роль рядка стосується номерів ЦЬОГО рядка: «Антігона», Креонт — це
				 * Креонт саме в «Антігоні». Перша названа лишається в `role` для
				 * картки без фільтра; тут — розкладка по уривках.
				 */
				if (play.role) {
					entry.roles ??= [];
					for (const item of play.items)
						if (!entry.roles.some((r) => r.item === item)) entry.roles.push({ item, role: play.role });
				}
			}
