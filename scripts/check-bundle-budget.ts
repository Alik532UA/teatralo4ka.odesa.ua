 ...
 *
 * 2026-08-29: 630 → 640. Поріг стояв упритул (629.9 із 630), і вітальне вікно
 * про переїзд галактики (`?update=open`) додало 2.1 КБ — 632.0.
 *
 * Перед підняттям порога перевірено, ЩО саме виросло, як і вимагає докблок
 * вище. Виріс рівно новий компонент; нічого стороннього в бандл не приїхало.
 * Сам текст оголошення в цих 2 КБ НЕ лежить: він у `static/galaxy/update-*.json`
 * і приходить запитом, коли вікно відкривають, — інакше два екрани тексту
 * потрапили б у словники i18n, які збираються цілком.
 *
 * Запас лишається вузьким свідомо (8 КБ, 1.3 %): це та сама відстань, на якій
 * наступний непомічений приріст знову стане червоним.
 *
 * Що тут насправді важить, якщо колись братися за схуднення: 167 КБ — маршрути
 * адмінки, ще 95 КБ — Firebase SDK. Разом це дві третини бюджету, і жодного з
 * них відвідувач галактики не завантажує. Критичний шлях головної при цьому
 * має 25 КБ запасу (295 із 320).
 */
const TOTAL_BUDGET_KB = 640;
