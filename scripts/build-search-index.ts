...
/* Відносний імпорт: цей скрипт виконує tsx, де аліасів SvelteKit немає. */
import { isLinked } from '../src/lib/config/visibility';
 ...
 * ## Кого сюди НЕ беремо
 *
 * Рівень `direct`, і тільки його. Це не лише про пошук: файл лежить у `static/`,
 * тобто його можна відкрити прямо. Скласти в нього текст такої людини означало б
 * віддати її одним запитом — рівно те, від чого `direct` і захищає.
 *
 * Рівень `linked` тут Є — те саме правило, що в `services/searchGalaxy`, і з тієї
 * самої причини: кожен `linked` уже показаний на публічній сторінці за зв'язком,
 * тож ховати його від пошуку означало б лише зробити пошук гіршим за очі.
 *
 * ## Свіжість
 *
 ...
 *
 * Спільна функція `isLinked` з `config/visibility`, а не власна копія умови:
 * доти тут стояло «`listed`, і ще учні як виняток», і виняток довелося б
 * дописувати на кожен новий вид людей. Так уже сталося з «друзями школи» —
 * автор 2026-09-08 запитав, чому пошук не знаходить Віктора Фурдуя.
 */
function уПошуку(запис: { visibility?: string }): boolean {
	return isLinked(запис);
}
