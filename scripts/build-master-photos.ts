...
const AVATAR_OVERRIDES: Record<string, CropOverride> = {
	...
	// Знімок на повний зріст, джерело 542×1080. Центральне обрізання взяло б
	// квадрат 542 від середини кадру — тобто пояс і цеглу, без обличчя. Голова
	// тут близько (270, 250), тож квадрат 260 від (140, 165) дає голову й плечі
	// з тим самим запасом над головою, що й у решти аватарів. Кадр не вгадано:
	// чотири кандидати відрендерено й порівняно.
	'hanna-tkach': {
		size: 260,
		sx: 140,
		sy: 165
	...
	// Вечірній знімок за столиком біля моря, джерело 960×1280. Центральне
	// обрізання взяло б квадрат 960 від (0, 38) — тобто море, поручні й ліхтарі,
	// а голова зайняла б близько чверті сторони. У списку аватар має 96px, тож
	// із такого плану лишилася б пляма — та сама причина, що в `hanna-podzihun`.
	// Обличчя близько x=580, y=560; квадрат 600 від (260, 360) лишає над головою
	// звичний запас (~12%) і доводить кадр до рук на столі.
	'tetiana-stepanchuk': {
		size: 600,
		sx: 260,
		sy: 360
...
const PORTRAIT_OVERRIDES: Record<string, { sx?: number; sy?: number; sw?: number; sh?: number }> = {
	...
	// Джерело 542×1080 (1:2), потрібне 2:3. Найбільший такий прямокутник —
	// 542×813, і взяти його треба ЗВЕРХУ: постать стоїть майже на всю висоту
	// кадру, тож центральне обрізання зрізало б і запас над головою, і черевики.
	// Так само лишається ціла голова, а внизу зникає сама лише підлога.
	'hanna-tkach': {
		sx: 0,
		sy: 0,
		sw: 542,
