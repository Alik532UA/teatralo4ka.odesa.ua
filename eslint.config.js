...
export default ts.config(
	...
	{
		/*
		 * `.cjs` — це CommonJS, і `require` там не борг, а єдиний спосіб.
		 *
		 * `no-require-imports` каже правду про модулі ESM: там `require` означає
		 * або недописану міграцію, або обхід статичного аналізу. Але файл із
		 * розширенням `.cjs` оголошує себе CommonJS явно, і `import` у ньому
		 * просто не працює — Node падає на розборі.
		 *
		 * У проєкті такий файл один: `lighthouserc.cjs`, і CJS він саме тому, що
		 * `@lhci/cli` вантажить конфіг через `require`. Він читає згенерований
		 * перелік адрес (`scripts/lighthouse-urls.ts`), і зробити це імпортом
		 * звідти неможливо.
		 *
		 * Заборона знята ФАЙЛОВОЮ маскою, а не рядком `eslint-disable` у місці
		 * виклику: рядок довелося б повторювати при кожному новому `require`, і
		 * перший же пропущений повернув би червоний lint на коді, який не має
		 * іншого вибору.
		 */
		files: ['**/*.cjs'],
		rules: {
