...
describe('поле пароля (INPUT-TOOLS-v8 § 3.1)', () => {
	...
	it('перевірка жива — компонент прочитано і кнопки в ньому є', () => {
		expect(src).toContain('InputTools');
	});

	it('не має кнопки копіювання', () => {
		const tools = /tools=\{\[([^\]]*)\]\}/.exec(src)?.[1] ?? '';
		expect(tools, 'набір кнопок не знайдено — перевірка перестала щось означати').not.toBe('');
		expect(tools, 'копіювання пароля кладе його в буфер обміну').not.toContain('copy');
	});
});
