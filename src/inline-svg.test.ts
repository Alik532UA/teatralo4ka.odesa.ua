...
/**
 * Сирий `<svg>` у розмітці сторінок (UI-UX-v9 § 3, MEDIUM).
 *
...
const ALLOWED = [
	...
	// компонента не дістає — SVELTE-UI-v9 § 3.5. Замінивши `<svg>` на
...
describe('сирий <svg> у розмітці', () => {
	...
	it('сторінки й компоненти беруть іконки з бібліотеки, а не вписують', () => {
		...
		expect(
			offenders,
			`сирий <svg> у розмітці (UI-UX-v9 § 3): візьміть іконку з lucide-svelte або винесіть у components/icons/\n${offenders.join('\n')}`
		).toEqual([]);
	});
