...
const VH_ON_PURPOSE: Record<string, string> = {
	...
	'src/lib/components/GraduateFormModal.svelte:height':
		'це ФОЛБЕК, а не вибір одиниці: наступним рядком стоїть той самий розмір ' +
		'у `dvh`, і кожен браузер, який знає нову одиницю, бере саме його — ' +
		'старий рядок для нього просто перезаписаний. Потрібен він тим, хто ' +
		'`dvh` не знає (Safari до 16.4): властивість із незрозумілою одиницею ' +
		'браузер відкидає ЦІЛКОМ, а це єдине вікно в проєкті, де висота задана ' +
		'через `height`, а не `max-height`, — без неї воно складається до висоти ' +
		'шапки, і анкета всередині перестає і скролитися, і натискатися. Ціна ' +
		'фолбека — смуга під згорнутою панеллю браузера саме в тих старих ' +
		'версіях; згорнуте вікно краще за складене',
