 ...
 * схема документа й нативні частини полів (UI-UX-v9 § 1.2, § 1.5.1).
 *
 ...
 * цитату за оголошення (AI-AGENT-PITFALLS-v9 § 1).
 */
...
describe('вигляд, який малює браузер', () => {
	 ...
	 *
	 * Зворотний експеримент (AI-AGENT-PITFALLS-v9 § 1.1): на попередній редакції
	...
	/**
	 * Те саме — але про ЗНАЧЕННЯ, які тег отримує вже в рантаймі.
	 *
	 * Перевірка вище дивилася лише на розмітку, і саме тому дефект прожив довго:
	 * у розмітці стояло чесне `light dark`, а скрипт першого кадру переписував
	 * його на `light` щоразу, коли обрано жовту тему. Тобто гейт обіцяв, що
	 * такого значення в проєкті немає, — а воно було, просто в іншому рядку.
	 *
	 * Місць, які пишуть у тег, рівно два: скрипт `app.html` і
	 * `applyThemeToDocument` у контролері. Обидва для світлих тем мусять ставити
	 * `only light`.
	 */
	it('ніде не ставиться голе «light»: ні в скрипті першого кадру, ні в контролері', () => {
		const джерела = [
			{ звідки: 'app.html', текст: APP_HTML },
			{
				звідки: 'controllers/ui.svelte.ts',
				текст: readFileSync(new URL('./lib/controllers/ui.svelte.ts', import.meta.url), 'utf8')
			}
		];
		const bad: string[] = [];
		for (const { звідки, текст } of джерела) {
			текст.split('\n').forEach((рядок, i) => {
				if (!/content/.test(рядок)) return;
				if (/['"]light['"]/.test(рядок)) bad.push(`${звідки}:${i + 1} — ${рядок.trim()}`);
			});
			if (!текст.includes('only light')) {
				bad.push(`${звідки}: ніде не ставить «only light» — світлі теми лишаться під Auto Dark`);
			}
		}
		expect(
			bad,
			`мета color-scheme отримує значення, яке вмикає Force Dark:\n${bad.join('\n')}`
		).toEqual([]);
	});
	...
	it('кожна з шести тем звужує схему, і селектор строго специфічніший за :root', () => {
		...
		for (const { name, scheme } of THEMES) {
			...
			/*
			 * СВІТЛІ ТЕМИ — САМЕ `only light`.
			 *
			 * Просте `light` каже Android Chrome «сторінка світла й своєї темної не
			 * має», і той перемальовує її Auto Dark Theme'ою. Автор надіслав це
			 * 7 вересня 2026: світлі теми на телефоні виходили перефарбованими до
			 * невпізнання. `only` — документована відмова, і для `light-dark()` вона
			 * не міняє нічого. Темним `only` не потрібне: темну сторінку Auto Dark
			 * Theme не чіпає.
			 */
			const очікуване = scheme === 'light' ? 'only\\s+light' : scheme;
			if (
				!own.some(({ body }) =>
					new RegExp(`color-scheme\\s*:\\s*${очікуване}\\s*;`).test(body)
				)
			) {
				problems.push(
					`${name}: очікувалося color-scheme: ${scheme === 'light' ? 'only light' : scheme}`
				);
			}
			// `html` обов'язковий: без нього специфічність (0,1,0) — нічия з `:root`,
			// і переможця обирає порядок правил, тобто бандлер
			// (SVELTE-UI-v9 `SUI-SCOPE-SPECIFICITY`). У сусідньому `as5` це заміряно:
 ...
 *
 * Зворотний експеримент (AI-AGENT-PITFALLS-v9 § 1.1): змінити будь-який
