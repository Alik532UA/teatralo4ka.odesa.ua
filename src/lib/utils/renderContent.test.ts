...
describe('getContentExcerpt', () => {
	...
	it('список і посилання разом не злипаються', () => {
		...
		expect(getContentExcerpt(md)).toBe('перший\nдругий');
	});

	it('абзаци лишаються окремими рядками, а не зливаються в стрічку', () => {
		// Знайдено на живій новині: оголошення стояло абзацами — дата, адреса,
		// кому адресоване, — а в описі картки виходила одна стрічка, де дата
		// злипалася з адресою: «…з 17 серпня 2026 Софіївська, 24 чекає…».
		const md = ['ОГОЛОШУЄМО НАБІР', '', 'з 17 серпня 2026', '', 'Софіївська, 24'].join('\n');
		expect(getContentExcerpt(md)).toBe('ОГОЛОШУЄМО НАБІР\nз 17 серпня 2026\nСофіївська, 24');
	});

	it('переноси всередині абзацу теж зберігаються', () => {
		// `<br>` у html із візуального редактора — той самий перенос.
		expect(getContentExcerpt('<p>Перший<br>Другий</p>', 'html')).toBe('Перший\nДругий');
	});

	it('зайві порожні рядки не множаться', () => {
		const md = ['А', '', '', '', 'Б'].join('\n');
		expect(getContentExcerpt(md)).toBe('А\nБ');
	});
