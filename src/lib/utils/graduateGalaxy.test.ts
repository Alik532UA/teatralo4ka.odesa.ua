 ...
 *
 * ## Зворотні експерименти (AI-AGENT-PITFALLS-v9 § 1.1)
 *
...
/**
 * Поле «пошук за прізвищем» у переліку випускників.
 *
 * Доти воно порівнювало підрядки власним `includes`, і три запити з чотирьох,
 * які людина справді набирає з пам'яті, давали порожній перелік. Тепер фільтр
 * ділить правило з пошуком по сайту (`utils/searchQuery`).
 */
describe('filterGraduates — прізвище набране не так', () => {
	const list = [
		graduate('Аліса Тункевич', 2016),
		graduate('Дар’я Гуревич', 2019),
		graduate('Марина Суханова', 1998)
	];
	const імена = (query: string) => filterGraduates(list, { year: 'all', query }).map((g) => g.name);

	it('інше написання тієї самої літери', () => {
		expect(імена('тункевіч')).toEqual(['Аліса Тункевич']);
		expect(імена('маріна')).toEqual(['Марина Суханова']);
	});

	it('прямий апостроф знаходить типографський', () => {
		expect(імена("дар'я")).toEqual(['Дар’я Гуревич']);
	});

	it('порядок слів не має значення', () => {
		expect(імена('тункевич аліса')).toEqual(['Аліса Тункевич']);
	});

	it('чужого прізвища не знаходить', () => {
		// Зворотний експеримент: згортання літер не має робити всіх схожими.
		expect(імена('гуревіч суханова')).toEqual([]);
		expect(імена('петренко')).toEqual([]);
	});
});
