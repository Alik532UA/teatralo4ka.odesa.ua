...
describe('focusableWithin', () => {
	...
	/**
	 * Рамка з чужим документом — теж зупинка Tab.
	 *
	 * Без цього вікно з анкетою Google замикало фокус на двох кнопках шапки, і
	 * до самої форми клавіатурою було не дійти: усе, заради чого вікно
	 * відкривали, лишалося недосяжним. Знайдено 2026-09-08 при розборі скарги
	 * «форма не скролиться й нічого не натискається».
	 */
	it('бере рамку з чужим документом — інакше в анкету не зайти', () => {
		const c = mount('<button>шапка</button><iframe title="анкета"></iframe>');
		expect(focusableWithin(c).map((e) => e.tagName)).toEqual(['BUTTON', 'IFRAME']);
	});
