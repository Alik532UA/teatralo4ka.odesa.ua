 ...
 * писалася руками — в анкеті Марини Суханової рядок 2011 року містив
 * «(разом з групою [ЗТК](…))» просто в тексті. Заміряно: рядків, де вистава
 * належить ще й чужій групі, **124**, і примітку мав рівно ОДИН. Тобто 123
 * такі самі рядки цього не казали, і кожна нова примітка руками означала б
 * ще один одинокий виняток.
 *
 * ## Чому порожньо, коли своїх груп не знаємо
 *
 * Із тих 124 рядків **82** належать випускникам, не прив'язаним до жодної
 * групи. Там кожна група вистави виглядає «чужою» лише тому, що своєї ми не
 * знаємо — а насправді людина майже завжди саме в ній і була. Плашка сказала б
 * «разом з Скоморохами» тому, хто сам Скоморох. Тому правило суворе: немає
 * своєї групи — немає й відповіді. Лишається 42 рядки (37 з однією групою, 4 з
 * двома, 1 з трьома).
 *
 * ## Чому реєстр, а не `classifyPlayGroups`
 *
 * Той рахує основні й допоміжні групи зі СКЛАДУ — для сторінки вистави, де
 * питання «чия це вистава». Тут питання інше й простіше: чи стоїть вистава в
 * репертуарі ще якоїсь групи. Це доглянутий факт із `playIds` — те саме, що
 * казала єдина примітка руками, і воно не залежить від того, скільки людей уже
 * встигли назвати виставу своєю.
 */
export function coGroupsForPlay(playId: string, memberId: string): GraduateGroup[] {
