...
describe('GROUPS data integrity', () => {
	 ...
	 */
	/*
	 * 2026-09-03: у перелік «щось на сторінці є» додано ЗНІМКИ.
	 *
	 * Причина не в тому, що перевірка комусь заважала, а в тому, що вона
	 * розійшлася з власним задумом. Задум — «на сторінці немає НІЧОГО»; але
	 * банер знімків це найбільший елемент сторінки групи, і сторінка з фотографією
	 * курсу, назвою та роком — не порожня адреса, а курс, про який поки відомо
	 * саме це.
	 *
	 * Проєкт цей стан уже моделює деінде: перелік груп має окрему категорію
	 * «Потребують уточнення» рівно для `memberIds.length === 0`. Тобто група без
	 * складу — очікуваний стан даних, а не дефект; заперечувала його лише ця
	 * перевірка.
	 *
	 * Що вона стереже далі — те саме: запис, у якому немає ні складу, ні
	 * репертуару, НІ знімків. Така адреса справді нічого не показує.
	 */
	it('сторінка групи не буває цілком порожньою', () => {
		const bad = GROUPS.filter(
			(g) => g.memberIds.length === 0 && g.playIds.length === 0 && (g.photos?.length ?? 0) === 0
		).map((g) => g.slug);
		expect(bad, `ні складу, ні репертуару, ні знімків:\n  ${bad.join('\n  ')}`).toEqual([]);
	});
