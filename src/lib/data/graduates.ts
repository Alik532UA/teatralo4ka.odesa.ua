...
/**
 * Порядок людей у переліку складу: спершу ті, хто школу ЗАКІНЧИВ.
 *
 * Прохання автора про сторінки фестивалів: «спочатку випускники, а потім ті,
 * хто не закінчив школу». Правило про ЛЮДЕЙ, а не про сторінку, тому живе тут,
 * поруч із самим `kind`: наступний перелік складу (групи, вистави) має брати
 * той самий порядок, а не вигадувати свій.
 *
 * Прохання називає дві групи, а значень чотири. Решта дві впорядковані тим
 * самим правилом «ближче до випуску — вище»: учень ще вчиться, тож іде за
 * тими, хто пішов; друг школи не вчився тут узагалі, тож останній.
 *
 * Відсутній `kind` — це випускник: у реєстрі поле проставлене лише винятками.
 *
 * Числа тут — не рейтинг людей, а порядок читання розділу; саме тому функція
 * повертає розряд, а не порівнює двох. Стійке сортування за ним лишає
 * всередині кожної групи той порядок, у якому список прийшов, — і саме на це
 * спирається перемішування складу на сторінці фестивалю.
 */
export function kindOrder(person: { kind?: GraduateKind }): number {
	if (person.kind === 'attended') return 1;
	if (person.kind === 'student') return 2;
	if (person.kind === 'friend') return 3;
	return 0;
}
