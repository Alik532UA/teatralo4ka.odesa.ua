...
/**
 * Порядок людей у переліку складу поїздки — чотири групи, і вісь не одна.
 *
 * Прохання автора, дослівно чотирма рядками:
 *
 *   1. у галактиці, з фотографією;
 *   2. у галактиці, без фотографії;
 *   3. поза галактикою, з фотографією;
 *   4. поза галактикою, без фотографії.
 *
 * ## Чому «у галактиці», а не «випускник»
 *
 * Попередня редакція ділила за `kind`, і автор поправив саме це: «ці, хто є в
 * галактиці, але не є випускником, теж зараховувати до цієї групи, таких як
 * Ігор Розводюк». Тобто значення має не те, ЧИ ЛЮДИНА ДОВЧИЛАСЯ, а те, чи
 * вона в переліку — тобто `visibility: listed`. Розводюк має `attended` і при
 * цьому летить у галактиці; за старим правилом він падав у другу групу, за
 * новим стоїть серед перших.
 *
 * ## Третя група порожня, і це навмисно
 *
 * Сьогодні поза галактикою з фотографією немає нікого — автор це знає й
 * попросив приготувати місце наперед: «таких немає, але архітектурно до цього
 * на майбутнє приготуватися». Тому розряд рахується з двох ознак, а не
 * перелічує три випадки з четвертим «решта»: порожня комірка тут коштує нуль
 * рядків, а поява першої такої людини не потребує правки.
 *
 * ## Чому розряд, а не порівняння двох
 *
 * Стійке сортування за цим числом лишає всередині кожної групи той порядок, у
 * якому список прийшов, — і саме на це спирається перемішування складу на
 * сторінці фестивалю: спершу тасуємо всіх, потім сортуємо стійко.
 */
export function rosterOrder(person: { visibility?: string; hasPhoto?: true }): number {
	return (isGraduateListed(person) ? 0 : 2) + (person.hasPhoto ? 0 : 1);
}
...
export interface GraduateIndexEntry {
	 ...
	 * «навчання до 2009», «навчається». Доти статус проходив через ключ перекладу
...
export interface GraduatePlay {
	 ...
	 * одному на уривок, бо роль живе в рядку: Алік Запольнов 2013 року —
	 * «Антігона», «Незнайомка» і «Сонети». Гейт дозволяє повтор вистави в
...
export interface GraduateProfile {
	...
	bio: string[];
	/**
	 * Знімки для галереї — кадри з вистав, курс, життя після школи.
	 *
	 * Це НЕ те саме, що `photoCount`: там портрети однієї людини різних років,
	 * обрізані в квадрат 96/192/480 під зірку в галактиці й аватарку в картці.
	 * Тут кадри будь-якої пропорції, які нічим не кадруються — їх показує
	 * `GroupPhotoBanner`, той самий, що на сторінках груп, вистав і фестивалів.
	 *
	 * Шляхи повні, від кореня `static/` — як `photos` у групи. Розміри кожного
	 * знімка мусять бути в `config/localImages.ts`: банер бере з них пропорцію,
	 * і гейт звіряє їх із самими файлами.
	 */
	gallery?: string[];
	festivals: string[];
 ...
 */
/**
 * Підпис під іменем — з `kind`: «випуск 2014», «навчання до 2009», «навчається».
 *
 ...
 * лишається без підпису.
 *
 * ПІДПИС БЕЗРОДОВИЙ, і це не обхід задачі. Була «навчався до 2009» — чоловіча
 * форма на всіх, зокрема на 60 % бази. Правильна пара «навчався / навчалася»
 * вимагає знати стать, а надійного джерела для неї немає: єдина наявна здогадка
 * (`utils/masterLabel`) читає закінчення прізвища й у власному докблоці
 * зізнається, що чоловічих правил не має — «Кореньок» і «Стельмах» вона
 * відрізнити не може й тихо повертає 'm'. Тобто вгадування дало б ту саму
 * помилку, лише складнішим шляхом.
 *
 * Тому підпис став ІМЕННИКОМ, як сусідні: «вступ», «випуск», «навчання до
 * 2009». Рід зникає з речення, а не вгадується.
 *
...
/**
 * УЧНІ — ті, хто вчиться зараз. Живуть на «Планеті творчості».
 *
 * Окремий перелік, а не частина `GRADUATES`, і це не оформлення: у галактиці
 * летять випускники й ті, хто перестав учитися, а поточний учень — інша
 * історія. Гейт `narrowed-fields` вимагає, щоб учень мав видимість `linked`
 * або `direct`, тобто в переліку галактики його й не буває; звідси й береться
 * цей зріз — із `LINKED_GRADUATES`, а не з усього індексу.
 *
 * Навіщо це заведено (словами автора): «легше запропонувати учням заповнювати
 * на себе анкети… щоб потім, коли прийде час, переводити їх у галактику
 * випускників». Переведення — це зміна `kind` і поява року випуску; АДРЕСА
 * людини при цьому не міняється, бо сторінка в неї від початку та сама.
 */
export const STUDENTS: readonly GraduateIndexEntry[] = LINKED_GRADUATES.filter(
	(g) => g.kind === 'student'
);
