 ...
 * не працює, не обов'язково показується в тому розділі — якщо її даних не
 * хватає, вона їде в «Потребують уточнення» (див. `masterSection`).
 */
...
/** Ініціал замість слова: «В.», «Д.Д.». Одна літера з точкою — не ім'я. */
const INITIAL = /^\p{Lu}\.(\p{Lu}\.)?$/u;

/**
 * Чи ПІБ повне: прізвище, ім'я та по батькові, і жодне з них не ініціал.
 *
 * Заміряно на всіх 118: 16 записів — лише прізвище («Якуніна»), 9 — без по
 * батькові («Капля Ірина»), 2 — з ініціалом замість нього («Рибакова Надія В.»).
 * Разом 27, і саме вони — головна причина, чому запис не може стояти в
 * змістовому розділі: підпис «Якуніна» не називає людину.
 *
 * Чотири токени («Діас Валдіс Дар'я Михайлівна») — складене прізвище, повне.
 */
export function hasFullName(fullName: string): boolean {
	const parts = fullName.trim().split(/\s+/);
	return parts.length >= 3 && !INITIAL.test(parts[parts.length - 1]);
}

/**
 * Чи даних достатньо, щоб показувати людину у змістовому розділі.
 *
 * Дві умови, і обидві — про те, чи картка взагалі називає людину: фотографія та
 * повне ПІБ. Посади (`roleTitle`) тут НЕМА навмисно, і це перевірено на даних:
 * її не має жодний із трьох записів «Світлої пам'яті» і шість із семи в
 * «Історії школи». Умова з посадою спорожнила б обидва розділи.
 */
function hasEnoughData(m: SectionInput): boolean {
	return Boolean(m.photo) && hasFullName(m.fullName);
}

/**
 * Рівно ті чотири поля, які читає `masterSection`.
 *
 * Вужче за `MasterIndexEntry` навмисно: так у сигнатурі видно, від чого розділ
 * ЗАЛЕЖИТЬ, і перевірити правило можна об'єктом із чотирьох полів, не вигадуючи
 * решти дванадцяти лише щоб задовольнити тип.
 */
export type SectionInput = Pick<MasterIndexEntry, 'fullName' | 'photo' | 'status' | 'category'>;
 ...
 *
 * Доти розділ лежав у `category` як значення `needsClarification`, і правило
 * («немає фотографії», плюс записи, названі поіменно) трималося на пам'яті того,
 * хто правив дані. Воно вже не діяло: фотографію мали 36 із 77 записів того
 * розділу, тобто головне правило було хибним для 47% його вмісту. Заповниш фото
 * — і треба не забути перенести категорію руками; гейта на це не існувало.
 *
 * ## Що змінилося в складі розділів — заміряно, а не припущено
 *
 * Правило нижче звірене з ручним поділом на всіх 118 записах: розходження в
 * ДЕВʼЯТИ, і кожне — на користь правила.
 *
 *   4 їдуть із «Історії школи» в «Потребують уточнення»: Ткач Ганна (немає
 *     фотографії), Шевчук Олег В., Рибакова Надія В. (ініціал замість по
 *     батькові), Стельмах (лише прізвище);
 *   5 навпаки — із «Потребують уточнення» в «Історію школи»: Імас Ольга,
 *     Домбровська, Мазур, Пруба, Серебряннікова Наталія. У всіх п'ятьох є і
 *     фотографія, і повне ПІБ; у технічному розділі вони стояли просто тому, що
 *     туди їх колись занесли руками.
 *
 * ## Порядок умов значущий
 *
 * `honorary` перевіряється ПЕРШИМ: «Світла пам'ять» — розділ про людей, а не про
 * повноту анкети, і виносити звідти когось за брак даних не можна. Далі повнота,
 * і лише потім цикл та роль: неповний запис мусить лежати в технічному розділі,
 * інакше перелік «що доробити» перестає бути переліком.
 *
 * Порожня `category` при повних даних теж дає «Потребують уточнення» — і це не
 * дірка, а те саме твердження: роль невідома, отже розділ ролі невідомий.
 */
export function masterSection(m: SectionInput): MasterSection {
	if (m.status === 'honorary') return 'honorary';
	if (!hasEnoughData(m)) return 'needsClarification';
