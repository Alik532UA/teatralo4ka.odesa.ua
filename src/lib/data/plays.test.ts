...
describe('реєстр вистав', () => {
	 ...
	 *
	 * `GraduateVideoButton` і `PlayRowVideoButton` малюють кнопку лише тоді, коли
	...
	const KNOWN_DISTINCT: readonly (readonly [string, string, string])[] = [
		...
		[
			'uryvky-z-klasyky-2015',
			'uryvky-z-klasyky-2015-3',
			'два покази: ДТШ курсом «ГруФФиш» (Ткач, номер 30) і ДТШ курсом 7Т-18 (Ісачкіна, 2 червня)'
		],
		[
			'uryvky-z-klasyky-2015-2',
			'uryvky-z-klasyky-2015-3',
			'два покази: училище курсом 4-РТЗ у Театрі на Чайній і ДТШ курсом 7Т-18 (Ісачкіна, 2 червня)'
		],
		[
			'uryvky-z-klasyky-2016',
			'uryvky-z-klasyky-2016-2',
			'два покази: ДТШ курсом «ТУ-154»/7Т-17 (номер 33, четверо майстрів) і ДТШ курсом 6Т-13 «Резонанс» (Ісачкіна) — програми не перетинаються'
		],
		[
			'pokaz-etiudiv-2024',
	...
	it('перевірка схожості жива: ловить саме те, через що наросли двійники', () => {
		/*
		 * Зворотний експеримент (AI-AGENT-PITFALLS-v9 § 1.1): пари, що справді
	...
	it('в основі складу немає тих, хто випустився до цього показу', () => {
		...
		expect(
			...
			'у ОСНОВІ складу стоїть той, хто вже випустився. Якщо він справді грав — ' +
				...
				'якщо це успадкування навмання — прибрати:' + bad.map((b) => `\n  ${b}`).join('')
		).toEqual([]);
	});
