 ...
 *
 * Для сімнадцятьох студентів 2026 року реєстр закладів — це ДРУГИЙ запис того
 * самого факту. Первинний живе рядком в анкеті: «Вступ 2026: КНУТКіТ,
 * акторський, курс Д. Богомазова». Обидва вписані руками, з однієї новини, в
 * один день — і саме тому вони роз'їдуться. Правити анкету легко (вона поруч із
 * людиною), а про сторінку закладу забувають; тоді КНУТКіТ показує шістьох, а
 * сьомий на своїй сторінці стверджує, що вступив саме туди. Помилка тиха:
 * обидві сторінки складаються.
 *
 * Тому перевірка «рядок анкети згадує цей заклад і цей рік» іде В ОБИДВА боки:
 * і кожна анкета зі словом «Вступ» мусить знайти свій заклад у реєстрі, і кожен
 * такий рядок мусить збігтися хоч з одним записом реєстру. Одна сторона
 * впустила б додану анкету, друга — забуту.
 *
 * Для решти ста шістдесяти студентів первинне джерело ІНШЕ — документ «Усі
 * випускники», де заклад стоїть біля прізвища, а в анкеті про нього нічого не
 * написано. Саме тому друга сторона перевірки не вимагає рядка від усіх: вона
 * впала б на сотні цілком правильних записів. Розбір — біля самої перевірки.
 *
...
describe('реєстр навчальних закладів', () => {
	...
	/*
	 * Року може не бути — розбір у докблоці поля `year`: для ДРУГОГО закладу
	 * поспіль джерело його не називає. Тому перевіряються межі, а не наявність.
	 *
	 * Але «немає ніде» теж мусить упасти: якби поле поділося з усіх записів,
	 * перевірка зеленіла б на порожньому переліку — звідси другий рядок.
	 */
	it('рік вступу, якщо він є, лежить у межах', () => {
		const bad: string[] = [];
		let зРоком = 0;
		for (const i of INSTITUTIONS)
		...
		expect(зРоком, 'жодного року в усьому реєстрі — поле зникло').toBeGreaterThan(50);
	});
	...
	/*
	 * Перевіряються ЛИШЕ ті, у кого рядок «Вступ …» в анкеті є.
	 *
	 * Джерел два, і в них різні ролі. Для сімнадцятьох студентів 2026 року
	 * первинне джерело — анкета: рядок «Вступ 2026: КНУКіМ, акторський, курс
	 * О. Печериці» вписано з новини, і реєстр закладів зібрано з нього. Для
	 * решти ста шістдесяти первинний — документ «Усі випускники», у якому
	 * заклад стоїть біля прізвища, а в анкеті про нього немає жодного слова.
	 * Вимагати рядка від усіх означало б зламати перевірку на самій сотні
	 * правильних записів.
	 *
	 * Умова тому інша: якщо анкета ПРО ВСТУП КАЖЕ, то хоч один запис реєстру
	 * мусить із нею збігтися. «Хоч один», а не «кожен», бо закладів у людини
	 * буває два (Катерина Петренко: ОТХФК і КНУТКіТ), а рядок в анкеті — один.
	 */
	it('де анкета каже про вступ, реєстр із нею збігається', () => {
		const bad: string[] = [];
		let перевірено = 0;
		const заЛюдиною = new Map<string, { name: string; year?: number }[]>();
		for (const i of INSTITUTIONS)
			for (const s of i.students)
				заЛюдиною.set(s.id, [...(заЛюдиною.get(s.id) ?? []), { name: i.name, year: s.year }]);

		for (const [id, записи] of заЛюдиною) {
			const graduate = byId.get(id);
			if (!graduate) continue; // назве попередня перевірка
			const line = afterGraduationOf(graduate);
			if (!line || !line.includes('Вступ')) continue;
			перевірено += 1;
			const збіг = записи.some(
				(з) => line.includes(з.name) && (typeof з.year !== 'number' || line.includes(String(з.year)))
			);
			if (!збіг)
				bad.push(
					`${graduate.name}: анкета каже «${line}», а реєстр — ` +
						записи.map((з) => `${з.name}${з.year ? ' ' + з.year : ''}`).join(', ')
				);
		}
		expect(bad, `реєстр закладів і анкета розійшлися:\n  ${bad.join('\n  ')}`).toEqual([]);
		expect(перевірено, 'жодної анкети зі вступом не перевірено').toBeGreaterThan(10);
	});
