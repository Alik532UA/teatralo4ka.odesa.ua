...
<style>
	...
	:global(.app.with-dynamic-bg) .deps {
	...
	/*
	 * Перекриття для темної теми ПРИБРАНО 2026-09-14, і це не спрощення.
	 *
	 * Воно з'явилося тоді, коли в основній темній `--text-title` дорівнював
	 * `--text-main`: заголовок картки не відрізнявся від її опису, і акцент був
	 * єдиним способом його виділити. Після обміну палітр із `dark-cyan` у темної
	 * з'явився власний колір заголовка — блакитний, — і перекриття почало
	 * робити протилежне задуманому: воно фарбувало заголовки в жовтий акцент
	 * кнопок, тобто зрівнювало їх із кнопками замість того, щоб виділяти.
	 *
	 * Тепер заголовок бере `--text-title`, як і решта заголовків сайту, у всіх
	 * шести темах. Це той самий селектор для «Вступити», «Контакти», «Історія»,
	 * «Музичне» й груп резидентів — усі три переліки цієї секції малюються ним.
	 */
