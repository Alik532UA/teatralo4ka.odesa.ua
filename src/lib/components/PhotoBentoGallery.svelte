<script lang="ts">
	...
	/**
	 * Знімок плитки. Ширший за `LightboxImage` на два поля, і кожне має причину:
...
<style>
	 ...
	 *
	 * Плитки без правил лишаються звичайними 1×1 — мозаїка розписана не на всі.
	 *
	 * Був ще й ЗВОРОТНИЙ виняток: поле `square` робило названу плитку 1×1
	 * усупереч її номеру. Жила воно рівно для восьмої плитки новини 4 вересня,
	 * де стояло портретне селфі, а восьма позиція широка. Коли автор поміняв той
	 * знімок місцями з п'ятим, потреба зникла — і поле, і правило прибрані, щоб
	 * у сітці лишився ОДИН спосіб задати розмір, а не два.
	 */
	...
	.g-bento__item--11 { grid-column: span 2; grid-row: span 1; }
