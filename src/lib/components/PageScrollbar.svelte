<script lang="ts">
	...
	/**
	 * Повзунок КІЛЬЦЕВИЙ: на шві він розрізається і видно обидва кінці.
	 *
	 * Вікно читача на шві справді стоїть у двох місцях одразу — низом на початку
	 * наступного кола, верхом на кінці поточного. Суцільний повзунок цього
	 * сказати не може, і обидві спроби виходили неправдою: обрізаний по низу
	 * ЗАМИРАВ на весь шов, а необрізаний перестрибував угору цілком, ніби читач
	 * миттєво опинився на початку.
	 *
	 * Тому дві частини однієї смужки: нижня доїжджає до краю доріжки, верхня
	 * рівно настільки ж виходить згори. Разом вони завжди дають `thumbHeight` —
	 * тобто розмір вікна не «дихає», просто перетікає через край.
	 *
	 * Поза зацикленням `thumbTop` ніколи не перевищує `maxThumbTop`, верхня
	 * частина виходить нульовою, і смужка лишається суцільною, як була.
	 */
	const thumbBelow = $derived(
		Math.max(Math.min(thumbHeight, viewportHeight - thumbTop), 0)
	);
	const thumbWrapped = $derived(Math.max(thumbHeight - thumbBelow, 0));
	...
	function applyScroll() {
		...
		/*
		 * Крайнє нижнє положення повзунка = ОСТАННІЙ ЕКРАН кола.
		 *
		 * Це виходить саме собою, бо `endless.pageHeight()` віддає висоту смуги:
		 * шкала тут така сама, як на звичайній сторінці, і жодного обрізання не
		 * треба. Проміжна редакція обрізала мету окремо й відбирала в повзунка
		 * саме крайнє положення — дотягнути його донизу ставало неможливо.
		 *
	...
	function endDrag() {
		...
		if (frame) {
			/*
			 * Кадр, що не встиг, ЗАСТОСОВУЄМО, а не викидаємо.
			 *
			 * Доти короткий клік по доріжці не робив нічого взагалі: натиск
			 * планує кадр через `requestAnimationFrame`, а відпускання в тому ж
			 * кадрі його скасовувало — тобто натиск повз повзунок гортав
			 * сторінку лише тоді, коли кнопку тримали довше за один кадр.
			 * Заміряно: `mousedown`+`mouseup` без паузи лишали прокрутку на
			 * місці, ті самі координати з паузою в 250 мс давали правильні 4222.
			 */
...
{#if enabled && !ui.isMenuOpen}
	...
	>
		<div
			...
			style="top: {thumbTop}px; height: {thumbBelow}px;"
			data-testid="page-scrollbar-thumb-status"
		></div>
		<!-- Частина, що перетекла через нижній край доріжки й вийшла згори.
		     Поза зацикленням її висота нульова, і елемента просто немає. -->
		{#if thumbWrapped > 0}
			<div
				class="page-scrollbar__thumb"
				style="top: 0; height: {thumbWrapped}px;"
				data-testid="page-scrollbar-wrap-status"
			></div>
		{/if}
	</div>
{/if}
