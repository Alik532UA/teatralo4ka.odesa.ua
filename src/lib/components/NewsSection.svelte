...
<section class="news" id="news-section" aria-labelledby="news-title" data-testid="news-section-container">
	<div class="container" data-testid="news-content-container">
		{#if items.length > 0}
			...
			/>
		{:else}
			<p style="text-align: center; color: var(--text-title); font-weight: bold; font-size: 1.2rem;" data-testid="news-empty-message">{$t('news.empty')}</p>
		{/if}
	</div>
</section>
{/if}
