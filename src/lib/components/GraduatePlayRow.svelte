<script lang="ts">
	 ...
	 *
	 * ## Як зверстано: назва з кнопкою — один шматок, плашка — окремий
	 *
	 * Тіло рядка — флекс із переносом. Перший елемент — назва разом із кнопкою
	 * запису, нерозривно: кнопка завжди праворуч від назви в першому рядку, хоч
	 * би якою довгою була назва. Другий — плашка «разом з»: у тому ж рядку,
	 * притиснута праворуч, коли вміщається; рядком нижче, теж праворуч, коли ні.
	 * Чому не завжди в одному й не завжди в різних — у докблоці `PlayRowGroups`.
	 * Уривки вечора — третім, на всю ширину, сходинкою під назвою.
	 *
...
<style>
	...
	/*
	 * Назва з кнопкою запису — ОДИН нерозривний шматок: кнопка завжди в першому
	 * рядку, хоч би якою довгою була назва. Без обгортки довга назва
	 * виштовхувала б кнопку в наступний рядок разом із плашкою.
	 *
	 * Шматок РОСТЕ (`flex: 1 1 auto`), і назва всередині теж: так кнопка стоїть
	 * біля правого краю рядка, а не липне до назви — у рядку з плашкою одразу
	 * перед нею, без плашки — на самому краю. Замовник просив саме праворуч:
	 * стовпчик кнопок читається, а розсип на різній відстані від назв — ні.
	 */
	.play__main {
		display: flex;
		align-items: start;
