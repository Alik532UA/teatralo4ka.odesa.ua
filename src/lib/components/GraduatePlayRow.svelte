...
<style>
	 ...
	 */
	/*
	 * Хвіст СТИСКАЄТЬСЯ, а не обрізається.
	 *
	 * Доти тут стояло `flex-shrink: 0`, і хвіст із чотирьох груп («Уривки з
	 * класики» 2016) виходив за край картки — зайві плашки просто зникали, без
	 * жодної ознаки, що вони є (знімок автора, 2026-09-03). Тепер він може
	 * звузитися, а плашка сама розкладає групи на рядки (`PlayRowGroups`).
	 *
	 * `min-width: 0` обов'язковий: типове `auto` у флексі означає «не вужче за
	 * вміст», і без нього стискання не діяло б попри `flex-shrink`.
	 */
	.play__tail {
		display: flex;
		align-items: center;
