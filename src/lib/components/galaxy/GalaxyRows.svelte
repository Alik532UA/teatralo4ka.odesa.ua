...
{#snippet row(item: GalaxyRow)}
	<li class="grow" data-testid="{testIdPrefix}-row-{item.key}">
		...
		<!--
			...
			Порожньої комірки-заповнювача тут НЕМА, і це рішення лишається: заповнювач
			коштував би 733 зайвих вузли на сторінці вистав — заміряно.

			Але міркування, яким його колись обґрунтували, було НЕПРАВИЛЬНЕ. Тут
			стояло: «остання колонка оголошена `auto`, тож без вмісту вона
			стискається в нуль, і плашки лишаються на правому краї байдуже, третьою
			вони стали чи четвертою». Колонка справді стискається в нуль — а от
			ПРОМІЖОК між колонками ні. Заміряно 2026-09-11 на переліку фестивалів:
			у рядків без облич плашки й прапор стояли на 12 px лівіше за решту —
			рівно на ширину проміжку, — і автор побачив це як невирівняні прапори.

			Тому плашки прив'язані до ОСТАННЬОЇ колонки явно (`grid-column: -2 / -1`
			у стилях), а не покладаються на те, якою за ліком вони опинилися.
		-->
...
<style>
	...
	.grow__cast {
		 ...
		 * стискає її, коли місця бракує, — у рядках груп вийшло 88 пікселів під
		 * 129 пікселів вмісту. Без обрізання обличчя лізли на плашки праворуч у
		 * 36 рядках із 36.
		 *
		 * Але ріже з ЗАПАСОМ: `overflow-clip-margin` лишає найближчі вісім
		 * пікселів, і саме в них збільшується кружечок під курсором. Доти тут
		 * стояв `hidden`, і в «Навчальних групах» він різав збільшене обличчя —
		 * тоді як у «Фестивалях» і репертуарі того самого не було, бо там рядок не
		 * лежить у колонці сітки.
		 *
		 * Підказка сюди не належить узагалі: вона у фіксованому шарі.
		 */
		overflow: clip;
	...
	.grow__marks {
		...
		/*
		 * ОСТАННЯ колонка явно, а не «яка дістанеться».
		 *
		 * Комірка складу малюється лише там, де є обличчя, тож у рядку без них
		 * плашки автоматично лягали в ТРЕТЮ колонку — і зупинялися на 12 px
		 * лівіше, бо проміжок до порожньої четвертої нікуди не дівається.
		 * Розбір і замір — у коментарі біля самої комірки складу.
		 *
		 * `-2 / -1` замість `4`: рахунок від кінця не доведеться правити, якщо
		 * колонок колись стане більше.
		 */
		grid-column: -2 / -1;
	}
	.grow__flags {
