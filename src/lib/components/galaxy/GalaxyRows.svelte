...
<style>
	...
	.grow__cast {
		 ...
		 * 36 рядках із 36.
		 *
		 * Кружечок під курсором тому й не виходить за межі: він збільшується
		 * усередину власного відступу рядка (див. `.mates` у `GraduateAvatarRow`),
		 * а підказка живе у фіксованому шарі й до цієї коробки не належить.
		 */
		overflow: hidden;
	}
