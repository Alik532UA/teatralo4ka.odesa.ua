<script lang="ts">
	...
	interface Props {
		...
		/**
		 * Рядок під пошуком — його вміст вирішує сторінка.
		 *
		 * Шапка вміла його показувати від початку (у груп і вистав там «показано
		 * N з M»), але через `GalaxyRegistry` він не проходив, тож сторінка, яка
		 * бере готовий перелік, лишалася без цього місця зовсім. Знадобилося
		 * майстрам і фахівцям — там під пошуком стоїть фільтр на три кнопки.
		 */
		scope?: Snippet;
		/**
