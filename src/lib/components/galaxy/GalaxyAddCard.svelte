...
>
	...
	<!--
		...
		Порядок читання: «Додати виставу → ось куди написати → а це навіщо».
		Стоячи в кінці, значки лишалися за поясненням, тобто дія — за її
		обґрунтуванням; людина, яка вже вирішила написати, мусила дочитати рядок,
		щоб дійти до кнопок.

		Розгорнуто, без олівця: прохання написати вже стоїть у назві, і кнопка
		...
		поруч уже стоїть «Напишіть мені — і показ з'явиться в архіві». Дві різні
		фрази про одну дію змушували читача вибирати, яка з них справжня.
	-->
	<EditContactButton
...
<style>
	...
	/* Коло, а не прямокутник: у значку один символ «+», і фігура мусить бути
	   такою самою в обидва боки. Розмір заданий ЧИСЛОМ, а не паддінгом —
	   паддінг навколо іконки 13px давав 25×19, тобто овал. */
	.add__badge {
		display: grid;
		place-items: center;
		flex-shrink: 0;
