<script lang="ts">
	 ...
	 *
	 * Тому рядок анкети — флекс із переносом (`GraduatePlayRow`): плашка стоїть у
	 * тому самому рядку, притиснута праворуч, а коли не вміщається —
	 * переноситься сама, не тиснучи назву. Тут від цього лише `margin-left:
	 * auto`: у спільному рядку він тягне плашку до правого краю, у власному —
	 * вирівнює її праворуч.
	 */
	interface Props {
