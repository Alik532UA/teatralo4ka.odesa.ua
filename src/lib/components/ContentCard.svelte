<script lang="ts">
	...
	export interface ContentCardItem {
		 ...
		 * 4 вересня 2026 автор просив зсунути знімок, і з першого разу я взяв
		 * напрям навпаки: більший відсоток веде вікно праворуч ПО ЗНІМКУ, а
		 * сам знімок від того їде ліворуч. Тому число тут беруть не на око, а
		 * з положення людей у кадрі — розрахунок стоїть біля значення в
		 * `config/codeNews.ts`.
		 *
...
<!--
	...
	Вона лежить УСЕРЕДИНІ посилання картки, і це наслідок свідомого рішення, а не
	недогляд: `AGENTS.md` («Клікабельність карток») вимагає, щоб уся картка була
	єдиним `<a>` без `::after` хаків. Кнопка гасить спливання (`preventDefault` +
	`stopPropagation`), тож натискання по ній не веде на сторінку новини.

	Тут довго стояв протилежний коментар — що картка це `<div>` з розтягнутим
	`::after`, бо інакше axe завалить збірку правилом `nested-interactive`.
	Обидва твердження застаріли після 32af46d, і друге ще й хибне: заміряно
	2026-08-31 на головній — правило `nested-interactive` ЗАПУСКАЛОСЬ і порушень
	не знайшло при 15 таких кнопках (0 порушень axe усього). Svelte у dev теж
	мовчить: 390 карток, 0 меж помилки. Ламає сторінку інший випадок — `<a>`
	всередині `<a>`; його стереже `e2e/nested-interactive.spec.ts`, і виняток для
	цієї кнопки там названий поіменно.
...
<style>
	...
	*/
	 ...
	 * (WCAG 2.2 SC 2.5.8, ACCESSIBILITY-v9 § 8). Іконка 14px із паддінгом
