...
<style>
	 ...
	 *
	 * ЗБІЛЬШЕННЯ ПІД ЧАС ПОЯВИ — окремою властивістю `scale`, а не в `transform`.
	 *
	 * `transform` тут зайнятий: у ньому живе наближення самої зірки при
	 * наведенні, і одне значення затирало б інше. Це та сама причина, через яку
	 * доріжка в `GraduateGalaxy` рухається `translate`, а не `transform`, — у
	 * цьому компоненті вже три різні перетворення на трьох різних елементах, і
	 * кожне мусить мати власну властивість.
	 *
	 * 0.72, а не «з розміру точки»: точка — це радіальний градієнт, чиє яскраве
	 * ядро близько шести пікселів, тож вирівнювати старт по її коробці 24 px
	 * нема сенсу — око порівнює зі світінням, а не з коробкою. Одне число на всі
	 * ранги ще й не дає розійтися трьом місцям, як уже було з діаметром обличчя.
	 *
	 * Крива — та сама `cubic-bezier(0.2, 0.8, 0.2, 1)`, що й у наведення вище:
	 * швидкий початок і довгий вихід. На спільній секунді це читається як
	 * «обличчя проявилося», а не як «щось під'їхало».
	 */
	...
	.star--multi .star__photos {
		...
		transition:
			opacity 1s ease-out,
			scale 1s cubic-bezier(0.2, 0.8, 0.2, 1);
	}
	...
	@media (prefers-reduced-motion: reduce) {
		 ...
		 * повного файлу (інакше повертається обличчя, обрізане горизонтально), а
		 * ось секунда перетікання й збільшення — це рух, і саме його просили
		 * прибрати.
		 *
		 * `transition: none` знімає ОБИДВІ властивості одразу, і `scale` окремо
		 * скидати не потрібно: без переходу елемент просто набуває значень
		 * готового стану — `opacity: 1`, `scale: 1` — у тому ж кадрі, коли
		 * з'явився клас. Інваріант `graduateStar.test.ts` стежить, щоб кожен
		 * елемент, який має перехід появи, був перелічений саме тут.
		 */
		.star:not(.star--multi) .star__photo,
		.star--multi .star__photos,
		.star__dot--waiting {
