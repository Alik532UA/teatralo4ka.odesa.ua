<script lang="ts">
	...
	interface Props {
		...
		fieldLabel?: string;
		/**
		 * Викликається після вставки й очищення, ЯКЩО поле має власний обробник
		 * введення. Слуг статті, наприклад, нормалізує роздільники в `oninput` —
		 * без цього виклику вставлене значення обійшло б нормалізацію й розійшлося
		 * б із тим, що бачить обробник.
		 */
