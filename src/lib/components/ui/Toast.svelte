<script lang="ts">
	...
	/**
	 * Два розміщення (NOTIFICATIONS-v1 § 5): глобальний стек у кутку для подій,
...
<style>
	...
	*/
	...
	.toast-card__excerpt {
		...
		display: -webkit-box;
		/*
		   Чотири рядки, а не три.

		   Обмеження тут не «скільки вміститься», а тверда межа: браузер ріже
		   рівно на вказаному рядку й домальовує три крапки, скільки б місця не
		   лишалося нижче. А місце лишалося: зображення виходить за поля тоста й
		   тому на 2rem вище за текстову колонку — приблизно два рядки порожнечі
		   під обірваним текстом.
		*/
		-webkit-line-clamp: 4;
		line-clamp: 4;
