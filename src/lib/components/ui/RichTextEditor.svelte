<script lang="ts">
	 ...
	 * нерозрізненний із забутою синхронізацією (SVELTE-CORE-v9 § 1.10).
	 */
...
<style>
	...
	/*
	 * Фокус показує КАРТКА, а не кожна редагована область (ACCESSIBILITY-v9 § 3).
	 *
	 * Усі чотири області вводу — візуальний редактор, `.ProseMirror`, HTML і
	 * Markdown — знімають кільце (`outline: none`), і не помилково: кожна займає
	 * усю картку, тож кільце було б другою лінією всередині рамки. Але доти
	 * заміни не було ЖОДНОЇ: `Tab` у редактор не змінював на екрані нічого.
	 * Рамка картки — та сама мова, якою фокус показують поля проєкту
