<script lang="ts">
	...
	import { ArrowDown, ArrowUp, ChevronDown, ChevronUp, Eye, EyeOff, Pencil, Save, Trash2 } from 'lucide-svelte';
	...
	// нічого б не перемалював (SVELTE-CORE-v9 § 1.5).
		...
		<div class="me-save-footer">
			...
			<div class="me-save-right">
				...
				<button type="button" class="btn-save-small {hasChanges ? 'is-active' : ''}" onclick={onsave} disabled={saving || !hasChanges} style="border: none;">
					{#if saving}{$t('admin.menuEditor.saving')}{:else}<Save size={18} style="margin-right: 0.5rem;" aria-hidden="true" /> {$t('admin.menuEditor.save')}{/if}
				</button>
			</div>
		</div>
	{/if}
