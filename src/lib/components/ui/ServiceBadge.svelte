<script lang="ts">
	 ...
	 *
	 * **Видимість (DEBUGGING-v9 § 2.1, із відхиленням).** У dev табло видиме ЗАВЖДИ,
	...
	async function copyReport() {
		try {
			...
			/*
			 * Але звіт не має зникати разом із відмовою (DEBUGGING-v9 § 2.3): текст
...
<style>
	 ...
	 * дотику (ACCESSIBILITY-v9 § 8, DEBUGGING-v9 § 2.2).
	 */
	@media (hover: none) {
		.badge {
			min-height: 44px;
