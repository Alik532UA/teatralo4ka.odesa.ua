<script lang="ts">
	 ...
	 * вони для лайтбокса на весь екран. Показати чотирнадцять таких у рамці
	 ...
	 *
	 * Тому в `static/galaxy/` лежать чотирнадцять мініатюр, зрізаних до 4:3 і
	 * зменшених до 216×162 (подвійний розмір рамки): разом 76 КБ. Той самий
...
>
	...
	>
		{#each КАДРИ as кадр, i (кадр.slug + кадр.file)}
			<!--
				`alt` порожній навмисно: підпис несе сама кнопка, а чотирнадцять
				описів поспіль читалися б з екранного диктора як чотирнадцять
