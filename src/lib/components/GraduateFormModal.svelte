<script lang="ts">
	...
	/**
	 * Заставка знімається або подією `load`, або ЧАСОМ.
	 *
	 * Подія — головний шлях, але покладатися лише на неї не можна: рамка чужа
	 * й міжсайтова, і якщо `load` не долетить (кеш, повернення з bfcache,
	 * блокувальник), заставка лишиться назавжди. А заставка — це непрозора
	 * панель на всю площу вікна: людина бачила б темний прямокутник замість
	 * анкети й не могла б ані прокрутити її, ані натиснути в ній хоч щось.
	 *
	 * Вісім секунд — не «скільки вантажиться форма», а «скільки ще має сенс
	 * показувати заставку»: після цього чесніше показати те, що є.
	 */
	const ЗАСТАВКА_МАКС_МС = 8000;

	$effect(() => {
		if (!isOpen) return;
		isLoading = true;
		const t = setTimeout(() => (isLoading = false), ЗАСТАВКА_МАКС_МС);
...
<style>
	...
	.form-modal {
		...
		/*
		 * ДВА рядки висоти, і другий не зайвий. `dvh` немає в Safari до 16.4
		 * (iPad, оновлений востаннє 2022-го, — цілком живий пристрій), а
		 * властивість із незрозумілою одиницею браузер відкидає ЦІЛКОМ. Тут це
		 * коштувало б дорожче, ніж деінде в проєкті: у решти вікон стоїть
		 * `max-height`, і без нього вони просто виростають, а тут — `height`, і
		 * без нього вікно складається до висоти шапки. Знадвору це виглядає
		 * рівно як «анкета не скролиться й нічого в ній не натискається».
		 */
	...
	/*
	 * `pointer-events: none` — обов'язково: заставка лежить НА рамці з анкетою,
	 * натискати на ній нема чого, а без цього правила вона перехоплювала б і
	 * дотики, і прокрутку. Тобто поки вона видна, форма під нею мертва — і саме
	 * так виглядає скарга «не скролиться й нічого не натискається».
	 */
	.form-modal__loader {
		position: absolute;
		inset: 0;
