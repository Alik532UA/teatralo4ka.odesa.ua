...
{#if уСтовпці.length > 0}
	<!--
		ОБГОРТКА, а не `grid-area` просто на `<aside>`, — і без неї стовпець
		наїжджає на галерею. Розбір у коментарі до `.article-media-slot` нижче.
	-->
	<div class="article-media-slot">
	<aside
		class="article-media"
		{@attach мірка}
...
<style>
	/*
	 * ДВА елементи замість одного, і це не зайвий `<div>`.
	 *
	 * Липкий стовпець мусить їхати рівно доти, доки триває ТЕКСТ, і зупинятися
	 * там, де починається галерея. Здавалося б, це вже задано сіткою: у
	 * `ArticleView` області розкладені як `'media text' / 'rest rest'`, тобто
	 * `media` займає лише перший рядок.
	 *
	 * Але для `position: sticky` область сітки нічого не значить. Контейнером,
	 * у межах якого липкий елемент має право їхати, є найближчий блоковий
	 * предок — тобто ВСЯ сітка разом із другим рядком. Тому стовпець спокійно
	 * доїжджав до галереї й лягав на неї згори.
	 *
	 * Заміряно на `/news/30th-season-opened-2026/`: при прокрутці 800 низ
	 * стовпця стояв на 675, а галерея починалася на 147 — перекриття 528 px, і
	 * далі воно росло до 2265.
	 *
	 * Обгортка це лагодить за побудовою: вона й займає область `media`, вона ж
	 * і є контейнером для липкого. `align-self: stretch` обов'язковий — сітка
	 * статті має `align-items: start`, і без нього обгортка була б заввишки зі
	 * свій вміст, тобто липкому не було б куди їхати взагалі.
	 *
	 * Вада не нова: вона просто була невидима, поки `position: sticky` не
	 * працював на сайті ніде через `overflow-x: hidden` на `body` (див.
	 * `global.css`). Щойно липкість полагодили — вилізло й це.
	 */
	.article-media-slot {
		grid-area: media;
		align-self: stretch;
