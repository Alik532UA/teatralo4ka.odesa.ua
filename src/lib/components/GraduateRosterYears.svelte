...
<style>
	...
	@media (max-width: 700px) {
		/*
		 * ВИСОТА СКИДАЄТЬСЯ, і без цього рядка список випускників був НЕДОСЯЖНИЙ.
		 *
		 * Базове правило має `height: 100%` — воно для розкладки, де панель стоїть
		 * ЗБОКУ від списку й мусить бути на всю його висоту. На вузькому екрані
		 * `.sheet__body` стає стовпчиком, і те саме `100%` означає вже інше: смуга
		 * років розтягується на всю висоту тіла й накриває собою рядки.
		 *
		 * Заміряно на 375×812: смуга виходила 594px замість ~60px, і клік по
		 * прізвищу діставався саме їй. Playwright писав про це прямо — «.years
		 * intercepts pointer events», — і чотири перевірки падали таймаутом, бо
		 * рядок був видимий, увімкнений і недосяжний.
		 *
		 * `flex: 0 0 auto` тут не косметика: без нього смуга в стовпчику знову
		 * почала б рости, тільку вже за рахунок `flex-grow`.
		 */
		.years {
			flex-direction: row;
			height: auto;
