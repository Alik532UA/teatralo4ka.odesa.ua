		...
		>
			{#each departments as dept, index (dept)}
				...
				>
					...
					{:else}
						<!--
							Підказка виїжджає ЗБОКУ від свого знака, у той
							бік, де є місце: знак із лівої половини ряду
							розкриває її ліворуч, із правої — праворуч.
						-->
...
<style>
	...
	/*
	 * Банер у плашці не додає власного повітря знизу: його дає сама плашка своїм
	 * `padding`. Типові відступи розраховані на сторінку-шапку, де під банером
	 * іде ще пів сторінки.
	 *
	 * Але між знімком і крапками пів рядка лишається. Нуль тут я вже ставив — і
	 * крапки сіли впритул до нижнього краю кадру, тобто візуально НА нього:
	...
	/*
	 * Контейнер «Фотографії» зафіксований по висоті й не змінюється (вимога:
	 * «по висоті фіксований і не змінюється»). Раніше пропорція бралася від найширшого
	 * знімка, і для вертикальних кадрів (наприклад, 9:16) плашка ставала завеликою (>600 px).
	 * Тепер рамка банера фіксовано має 240 px, а кадри будь-якої орієнтації
	 * акуратно вписуються по центру без стрибків висоти.
	 */
	.bento-card--gallery {
		--banner-gap: 0.5rem;
		--banner-dots-gap: 0;
	...
	.bento-card--gallery :global(.banner) {
	...
	@media (min-width: 768px) {
		...
		.col {
			...
			box-shadow: none;
			/*
			 * ПОЛЕ ПІД ТІНЬ — ТІЛЬКИ ЛІВОРУЧ, і це не половинчастість.
			 *
			 * `overflow-y: auto` робить колонку вікном прокрутки, а вікно
			 * прокрутки обрізає тінь плашки: на заокругленому куті прямий зріз
			 * читається трикутником — саме це надіслав автор. Поле відсуває межу
			 * обрізання, від'ємний відступ повертає плашки на місце.
			 *
			 * Перша редакція дала поле з УСІХ боків — і зламала те, чого я не
			 * врахував: смуга прокрутки малюється по ПРАВОМУ краю цього самого
			 * блока, тож разом із ним поїхала й вона. Автор побачив одразу.
			 *
			 * Тому праворуч поля немає: там смуга, і її край має лишатися там,
			 * де його намалював автор. Зверху й знизу теж — вертикальний розмір
			 * колонки задає модалка, і вихід за неї коштував би дорожче за
			 * виграш. Лишається ліворуч — саме той бік, де зріз і видно: там
			 * тінь лежить на порожньому тлі, а не на сусідній колонці.
			 */
			padding: 0 0 0 40px;
			margin: 0 0 0 -40px;
			display: flex;
	...
	.dept-badges {
		...
		gap: 0.4rem;
		/*
		 * ПОСЕРЕДИНІ між знімком і іменем.
		 *
		 * Доти весь просвіт належав сусідові згори (`margin-bottom: 1.1rem` у
		 * знімка), а сам ряд не мав жодного: він прилипав до імені й висів
		 * далеко від фотографії. Тепер відстань поділена навпіл, а сусід згори
		 * віддає свою — тож загальна висота шапки не змінилася й ніщо нижче не
		 * поїхало.
		 */
		margin: 0.55rem 0;
	}
	/*
	 * Просвіт віддає САМЕ ТОЙ сусід, за яким іде ряд знаків, — і лише тоді,
	 * коли ряд є: відділень у людини може не бути зовсім, і тоді знімок мусить
	 * лишитися з повним відступом до імені.
	 *
	 * `:has()`, а не від'ємний відступ згори: у знімка, квітки й зірки різні
	 ...
	 * потрібний бік.
	 *
	 * ## ТОЧКА ВІДЛІКУ — САМ ЗНАК, І ПІДКАЗКА ВИЇЖДЖАЄ ЗБОКУ
	 *
	 * Тут було два хибні варіанти поспіль, і обидва варто лишити записаними.
	 *
	 * ПЕРШИЙ: точкою відліку був РЯД знаків, і підказка кріпилася до його
	 * лівого чи правого краю — тобто до краю картки. Наводиш на лівий знак, а
	 * назва спалахує аж біля протилежного краю, ще й обрізана. Формально вона
	 * «лишалася всередині картки», по суті — не належала знакові, на який
	 * дивляться.
	 *
	 * ДРУГИЙ: підказка виїжджала ВНИЗ із-під знака. Прив'язку це полагодило, а
	 * читалося погано: назва лягала на ім'я під нею, і замість підпису до знака
	 * виходила накладка посеред картки.
	 *
	 * Тепер вона виїжджає ЗБОКУ від свого знака, у той бік, де є місце: знак із
	 * лівої половини ряду розкриває її ліворуч, із правої — праворуч, тобто
	 * назовні від сусідів.
	 *
	 * Один випадок таки лишається тісним: у ТРЬОХ відділень середній знак
	 * вільного боку не має, і підказка накриває сусіднє коло. Полічено в
	 * реєстрі — три відділення рівно в однієї людини з 543 (у 35 їх двоє, у 500
	 * одне), тож заради цього випадку вигинати решту 542 не варто; поки на знак
	 * наводять, сусіднє коло під підказкою нікому не потрібне.
	 *
	 * Ширина береться від ПЛАШКИ (`cqw`, контейнер оголошено на
	 * `.bento-card--main`), а не у відсотках: відсотки рахувалися б від власного
	 * кола в 32 px, від половини якого не лишилося б нічого.
	 *
	...
	.dept-badge__tip {
		position: absolute;
		/* Прив'язка — сам знак (`.dept-badge` уже `position: relative`). */
		top: 50%;
		left: calc(100% + 0.35rem);
		translate: 0 -50%;
		/*
		 * Вище за сусідні кола. `.dept-badge` має `position: relative` без
		 * власного `z-index`, тобто свого контексту накладання не створює, — і
		 * підказка з `5` лягає поверх усіх знаків ряду, а не лише попередніх.
		 */
		z-index: 5;
		max-width: 0;
		overflow: hidden;
	...
	/* Знак із лівої половини ряду розкриває підказку ЛІВОРУЧ — від себе. */
	...
	.dept-badge:focus-visible .dept-badge__tip {
		/*
		 * Від краю знака до краю плашки — приблизно її половина за відніманням
		 * стовпчика знаків; 38 % лишають запас на внутрішній відступ картки.
		 * `min()` тримає підказку розумною й тоді, коли плашка широка.
		 */
		max-width: min(38cqw, 10.5rem);
	...
	/*
	 * БЕЗ `transform` під курсором — сяйво замість збільшення.
	 *
	 * Спершу причина була інша: підказка кріпилася до краю РЯДУ, а
	 * трансформований елемент стає системою координат для своїх абсолютних
	 * нащадків, тож збільшення знака зривало її на 145 px убік — заміряно.
	 * Тепер підказка й так належить знакові, тобто ця пастка зникла.
	 *
	 * Збільшення все одно не повертається: воно зсувало б і саму підказку рівно
	 * в мить, коли її читають. Сяйво каже те саме («ця плашка жива»), нічого не
	 * роблячи з координатами.
	 */
	.dept-badge:hover {
