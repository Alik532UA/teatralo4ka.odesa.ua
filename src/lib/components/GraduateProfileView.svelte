...
<style>
	...
	@media (min-width: 768px) {
		...
		.col {
			...
			box-shadow: none;
			/*
			 * ПОЛЕ ПІД ТІНЬ ПЛАШОК, і воно ж повертається від'ємним відступом.
			 *
			 * `overflow-y: auto` робить колонку вікном прокрутки, а вікно
			 * прокрутки ОБРІЗАЄ все, що виходить за його межі, — зокрема тінь
			 * плашки. Видно це не як «тіні немає», а гірше: тінь є, і в неї
			 * рівний прямий зріз по краю колонки. На заокругленому куті зріз
			 * читається трикутником — саме це й надіслав автор.
			 *
			 * Числа з самої тіні, а не з ока: `--shadow-main` у темних темах це
			 * `0 10px 40px`, тобто вона сягає 40 px убік, 30 угору (40 − 10
			 * зсуву) і 50 вниз. Від'ємний відступ рівно на це поле повертає
			 * плашки на місце: рухається тільки МЕЖА ОБРІЗАННЯ.
			 *
			 * Ціна названа: коробки сусідніх колонок перекриваються (проміжок
			 * сітки 25.6 px, а поля дають 80), тож прокрутка колесом У ПРОМІЖКУ
			 * дістається тій колонці, що пізніша в розмітці. Плашок це не
			 * стосується — вони стоять у своїх полях.
			 */
