...
<style>
	...
	.header__logo-link {
		...
		line-height: 0;
		/*
		 * Натискання ловить сам знак, а не коробка навколо нього — див. докблок
		 * у `LogoIcon`. Тут те саме й на посиланні: воно обгортає ту саму
		 * коробку, тож без цього рядка перехоплювало б кліки нижче знака.
		 * Подія від знака сюди спливає, тож перехід на головну працює.
		 */
		pointer-events: none;
	}
	...
	.header__settings-popover {
		...
		/* Прокрутка мусить бути саме ТУТ, а не в кожній групі всередині.
		 *
		 * Попов містить ДВА окремі блоки — мову з темою і налаштування нижче, —
		 * і за екран вилазить їхня спільна висота. Обмеження на кожному блоці
		 * окремо дало б дві незалежні смуги замість однієї спільної, а нижній
		 * блок усе одно лишався б за краєм.
		 *
		 * Висота рахується від того, що насправді зайнято: попов починається під
		 * шапкою (`--header-height` плюс власний `padding-top`), а знизу лишає
		 * запас на відступ. Попереднє «мінус 6rem» не враховувало власного
		 * зсуву попова, тож він доходив рівно до низу екрана — і нижній пункт
		 * усе одно лишався зрізаним, попри наявну прокрутку.
		 *
		 * 100dvh, а не vh: на мобільних панель браузера з’їдає частину екрана. */
		/* Ширина — тут, бо саме попов її й роздає дітям (правило нижче). Доти
		   вона бралася від найширшої картки, тобто залежала від того, яка з
		   двох випадково більша. */
		width: 262px;
		box-sizing: border-box;
		--popover-reserve: calc(var(--header-height, 72px) + 25px + 1.5rem);
		max-height: calc(100dvh - var(--popover-reserve));
		overflow-y: auto;
		/* Щоб тінь групи не зрізалася об край області прокрутки. */
		padding-right: 2px;
	}

	/* Усередині попова власна прокрутка групам не потрібна: вона одна на всіх,
	   інакше вийшли б вкладені смуги. */
	.header__settings-popover :global(.dropdown-menu-unified) {
		max-height: none;
		overflow-y: visible;
		/*
		 * ШИРИНУ ЗАДАЄ ПОПОВ, А НЕ КОЖНА КАРТКА ОКРЕМО.
		 *
		 * Доти обидві мали власне число: 220px у мови з темою і 260px у
		 * налаштувань нижче. Звіряти їх не було кому, і на екрані це виглядало
		 * як два накладені меню — нижня картка ширша на 40px, вилазить праворуч
		 * і накриває кут верхньої. Заміряно: 916…1136 проти 916…1176.
		 *
		 * Дві картки — це намір (мова з темою окремо від решти), а різна
		 * ширина — ні.
		 */
		width: 100%;
	}

	/* Підвал приклеєний до низу лише тут — саме він і накривав нижні пункти. */
