<script lang="ts">
	 ...
	 */
	/*
	 * Значення відоме ДО першого рендеру в браузері, а не після нього.
	 *
	 * З `false` на старті гілка перемикалася вже після монтування, і поки старе
	 * піаніно зникало переходом, нове вже стояло поруч — на 420 мс у розмітці
	 * було ДВА `footer-piano-btn`. Спіймав це `testid.spec`, і правильно: два
	 * однакові testid означають, що клік у тесті може потрапити не в ту кнопку.
	 */
	let sideBySide = $state(
		browser ? window.matchMedia('(max-width: 768px)').matches : false
	);
