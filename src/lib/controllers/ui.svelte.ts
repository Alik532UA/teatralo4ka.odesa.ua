...
export class UIState {
	 ...
	 *
	 * **Типове значення `true`**, і це свідома зміна рішення, а не недогляд.
	 *
	 * Перша редакція ставила `false` — за зразком `holdScroll` вище, бо механізм
	 * міняє найбазовішу обіцянку сторінки: що в неї є кінець. Аргумент лишається
	 * правильним для ДОВОДКИ, і там типове значення не змінилося. Для зациклення
	 * він хибний, і різниця конкретна: доводка рухає сторінку БЕЗ вводу (курсор
	 * стоїть — сторінка їде), тобто робить те, чого читач не просив. Зациклення
	 * не рухає нічого саме: воно лише відповідає на власний рух читача, який
	 * докрутив до низу, тим, що там є ще. Це властивість сторінки, а не дія над
	 * людиною.
	 *
	 * Практичний бік вирішив те саме: при типовому `false` автор двічі повідомив
	 * «головна закінчується» — тобто настройка, якої не видно, читалася як
	 * незроблена робота. Перемикач лишається на місці й тепер ВИМИКАЄ; шлях до
	 * обох станів однаково досяжний, тож закид `PS-REACHABILITY` знято так само.
	 *
	 * Діє лише на десктопі й лише на головній — умови живуть у
	 * `controllers/endless`, бо це властивість розкладки, а не настройки.
	 * Тумблер показується теж за ними: перемикач, що нічого не змінює, гірший
	 * за його відсутність.
	 *
	 * У скрипт першого кадру не йде: на перший кадр не впливає, бо розгін
	 * зʼявляється вже після монтування, і на головній його ще й прикриває
	 * заставка. Типового значення від адміністратора немає — воно завжди `true`.
	 */
	endlessScroll = $state(true);

	/**
	 * Чи діють одиночні літерні скорочення сайту (`T` тема, `L` мова).
	 *
	 * **Це виконання WCAG SC 2.1.4 «Character Key Shortcuts», рівень A**
	 * (HOTKEYS-v9 § 3, `HK-WCAG-CHARACTER-KEY`, CRITICAL). Критерій вимагає
	...
	constructor() {
		if (typeof window !== 'undefined') {
			...
			// Те саме правило читання, що й у двох попередніх: значення, яке не
			// є ані 'true', ані 'false', означає «не сказано».
			const endless = storage.get('endlessScroll');
			if (endless === 'true' || endless === 'false') {
				this.endlessScroll = endless === 'true';
			}
	...
	setEndlessScroll = (on: boolean) => {
		this.endlessScroll = on;
		storage.set('endlessScroll', on ? 'true' : 'false');
	};
