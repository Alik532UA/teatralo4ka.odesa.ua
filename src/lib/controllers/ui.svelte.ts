...
export class UIState {
	 ...
	 *
	 * **Типове значення `false`**, і з тієї самої причини, що в `holdScroll`
	 * вище: механізм міняє найбазовішу обіцянку сторінки — що в неї є кінець.
	 * Читач, який цього не просив, не має способу здогадатися, чому галерея
	 * знову змінилася на шапку, і де це прибрати. Тому вмикає його сам —
	 * тумблером у тому ж меню смуги, де вибирає режим.
	 *
	 * Це ЄДИНИЙ спосіб дійти до ввімкненого шляху, і саме він знімає закид
	 * `PS-REACHABILITY` («вимкнений механізм читається як зроблена робота»):
	 * без видимого перемикача правильним ходом було б не робити нічого.
	 *
	 ...
	 * зʼявляється вже після монтування. Типового значення від адміністратора
	 * теж немає — воно завжди `false`.
	 */
	endlessScroll = $state(false);
