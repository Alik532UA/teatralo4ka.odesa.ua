...
class EndlessState {
	...
	/**
	 * Чи читач уже НА ШВІ — тобто в кадрі стоїть початок наступного кола.
	 *
	 * Потрібне тим, хто питає не «де я на шкалі», а «чи я вгорі». Кнопка
	 * «нагору» саме така: за глибиною читач на шві найдальший від початку, а на
	 * екрані в нього герой. Пропозиція повернутися нагору там читається як
	 * помилка — повертатися нема звідки.
	 *
	 * Аргумент — уже пораховане `lapY`, а не сира прокрутка: той, хто питає,
	 * однаково його має, і другий вимір розійшовся б із першим на кадр.
	 */
	inSeam(lapY: number): boolean {
		return this.active && lapY > this.lastScreen;
	}
