---
title: "«Одеса.Театр.PRO»"
date: "2026-04-17"
author: "Alik"
category: "projects"
lang: "uk"
seo:
  title: "Фестиваль «Одеса.Театр.PRO» - Одеська театральна школа"
  description: "Відкритий фестиваль «Одеса.Театр.PRO» для абітурієнтів творчих вишів. Проводиться Одеською театральною школою."
status: "published"
version: "1.1.0"
excerpt: "Фестиваль абітурієнтів закладів мистецької освіти України"
coverUrl: "/2025-2026/teatr-pro-2026.jpg"
---

[<svg role="img" aria-label="Зателефонувати" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 8px; display: inline !important;"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.79 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l2.28-2.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>](tel:+380631509551) [<svg role="img" aria-label="Написати на пошту" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 8px; display: inline !important;"><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"></path><rect width="20" height="16" x="2" y="4" rx="2"></rect></svg>](mailto:teatr.pro.fest@gmail.com) [<img src="/social_media/facebook-se-512-50.png" style="width: 32px !important; height: 32px !important; display: inline !important; border-radius: 0 !important; vertical-align: middle; margin: 0 8px 0 0 !important;" alt="facebook" />](https://www.facebook.com/groups/Odesa.Teatr.PRO/) [<img src="/social_media/instagram-se-512-50.png" style="width: 32px !important; height: 32px !important; display: inline !important; border-radius: 0 !important; vertical-align: middle; margin: 0 8px 0 0 !important;" alt="instagram" />](https://www.instagram.com/go_pro.teatr/) [<img src="/social_media/YouTube-se-512px-50q.png" style="width: 32px !important; height: 32px !important; display: inline !important; border-radius: 0 !important; vertical-align: middle; margin: 0 8px 0 0 !important;" alt="youtube" />](https://www.youtube.com/@odesateatrpro) [<img src="/social_media/Telegram-se-320px-50q.png" style="width: 32px !important; height: 32px !important; display: inline !important; border-radius: 0 !important; vertical-align: middle; margin: 0 8px 0 0 !important;" alt="telegram" />](https://t.me/odesateatrpro)

# «Одеса.Театр.PRO»

> **PRO**водиться Одеською театральною школою\
> за підтримки Департаменту культури, туризму та охорони культурної спадщини Одеської міської ради\
> Національної спілки театральних діячів України\
> для абітурієнтів творчих навчальних закладів,\
> які мріють стати акторами, режисерами, медійниками\
> та спеціалістами інших творчих **PRO**фесій

### **PRO**читай свою програму!
### **PRO**качай свої навички! 
### с**PRO**буй свої сили! 
### Не **PRO**гав, не **PRO**пусти, не **PRO**спи!

---

Ваша вступна **PRO**грама буде оцінюватися **PRO**фесіоналами (в Експертній раді фестивалю — майстри з Києва, Львова, Харкова, Дніпра та Одеси, які цього року набирають перший курс).

Тут ви отримаєте **PRO**гноз щодо вступу та **PRO**сунений рівень театральної кваліфікації.

### **PROбуджуйся, PROкидайся, PROбивайся!**

Акторські **PRO**би на фестивальній сцені,\
**PRO**гресивні методики та **PRO**відні фахівці,\
**PRO**двинуті тренінги та якісний творчий **PRO**дукт,\
сучасний театр та його **PRO**блематика,\
**PRO**гулянки Одесою та **PRO**сто крута театральна туса.\
Все це — у фестивальному **PRO**єкті «Театр.PRO».

Ви можете стати учасником або слухачем фестивалю та отримати відповідний Сертифікат про підвищення кваліфікації.


> За**PRO**шуємо до Одеси\
> Teatr.PRO - це твій театральний **PRO**стір!

---

**[Одеська театральна школа](https://maps.app.goo.gl/ya4gki6tuZv36Tjz8)**\
**[вул. Софіївська, 24, Одеса](https://maps.app.goo.gl/ya4gki6tuZv36Tjz8)**

**[+380 48 723 63 04](tel:+380487236304)**\
**[+380 48 723 32 59](tel:+380487233259)**\
**[+380 63 150 95 51](tel:+380631509551)**

[<svg role="img" aria-label="Написати на пошту" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 8px; display: inline !important;"><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"></path><rect width="20" height="16" x="2" y="4" rx="2"></rect></svg> teatr.pro.fest@gmail.com](mailto:teatr.pro.fest@gmail.com)\
\
[<img src="/social_media/facebook-se-512-50.png" style="width: 32px !important; height: 32px !important; display: inline !important; border-radius: 0 !important; vertical-align: middle; margin: 0 8px 0 0 !important;" alt="facebook" /> facebook Odesa.Teatr.PRO](https://www.facebook.com/groups/Odesa.Teatr.PRO/)\
\
[<img src="/social_media/instagram-se-512-50.png" style="width: 32px !important; height: 32px !important; display: inline !important; border-radius: 0 !important; vertical-align: middle; margin: 0 8px 0 0 !important;" alt="instagram" /> instagram go_pro.teatr](https://www.instagram.com/go_pro.teatr/)\
\
[<img src="/social_media/YouTube-se-512px-50q.png" style="width: 32px !important; height: 32px !important; display: inline !important; border-radius: 0 !important; vertical-align: middle; margin: 0 8px 0 0 !important;" alt="youtube" /> youtube @odesateatrpro](https://www.youtube.com/@odesateatrpro)\
\
[<img src="/social_media/Telegram-se-320px-50q.png" style="width: 32px !important; height: 32px !important; display: inline !important; border-radius: 0 !important; vertical-align: middle; margin: 0 8px 0 0 !important;" alt="telegram" /> telegram odesateatrpro](https://t.me/odesateatrpro)

<!--
Оголошення набору — прибрано зі сторінки після фестивалю 5–6 червня 2026,
але лишається тут ДОСЛІВНО: наступний випуск оголошується тим самим
текстом і, найімовірніше, тими самими заявками. Щоб повернути — вийняти
потрібне з цього коментаря й замінити рік, номер випуску та адреси форм.

[<svg role="img" aria-label="Зателефонувати" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 8px; display: inline !important;"><path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M10 9H8"/><path d="M16 13H8"/><path d="M16 17H8"/></svg> Положення фесту ОдесаТеатрPRO-2026](https://drive.google.com/file/d/1WbnasH0tSiDQpp-E8hMZQDd1B4A8svpi/view)

## ІV-й відкритий фестиваль

## 5-6 червня 2026

### Заповнюй заявку тут!

<div style="display: flex; flex-direction: column; gap: 1rem; margin: 1.5rem 0;">
  <a href="https://forms.gle/6yDCeujpkySSfE9p9" class="btn btn-primary" style="text-transform: none; text-decoration: none; width: fit-content;">Заявка учасника</a>
  <a href="https://forms.gle/UCBsLYq43TuypGqu6" class="btn btn-primary" style="text-transform: none; text-decoration: none; width: fit-content;">Заявка слухача</a>
</div>
-->
