import { LINKED_GRADUATES, graduateAddress, graduateProfilePath } from '$lib/data/graduates';
 ...
 * ## Кого сюди НЕ беремо
 *
 * Рівень `direct` — і тільки його. Це «таємний агент» із моделі видимості
 * (`config/visibility.ts`): сторінка жива, але не показується НІДЕ, і знайти її
 * можна лише прямим посиланням. Саме цього й просили для Володимира Захарченка,
 * і саме це правило його й береже.
 *
 * ## Чому `linked` тепер У пошуку
 *
 * Доти сюди йшов лише `listed` плюс окремо учні. Автор 2026-09-08 запитав, чому
 * пошук не знаходить Віктора Фурдуя — він `linked` і стоїть на
 * сторінці фестивалю «Південні маски» 2009. Заміряно по всіх п'ятьох `linked`
 * не-учнях: КОЖЕН із них уже показаний на публічній сторінці — четверо на
 * фестивалях, один у складі вистави. Тобто виключення з пошуку нічого не
 * ховало: ім'я й так видно, просто власний пошук сайту знаходив гірше, ніж
 * очі.
 *
 * Модель видимості говорить про `linked` дослівно: «не в переліку, але скрізь,
 * де на людину є зв'язок». Пошук за іменем — це і є випадок «знаю ім'я, дайте
 * сторінку», а не перелік. Тому тепер джерело одне — `LINKED_GRADUATES`, тобто
 * всі, крім `direct`; окремий рядок для учнів зник разом із винятком, бо учні
 * потрапляють сюди тим самим загальним правилом.
 *
...
/**
 * Підпис за видом людини — те саме слово, що на її сторінці.
 *
 * Без цього всі йшли як «випуск»: Віктор Фурдуй, який відділення не закінчував,
 * підписувався випускником рівно в тому рядку, де людина перевіряє, чи то саме
 * той, кого вона шукає.
 */
const ПІДПИС: Record<string, string> = {
	student: 'учень школи, навчається',
	attended: 'навчався в школі',
	friend: 'друг школи',
	intensiveCourse: 'інтенсивний курс',
	festivalGuest: 'участь у фестивалі'
};
...
export function galaxyEntries(
	...
	for (const g of LINKED_GRADUATES) out.push(людина(g, ПІДПИС[g.kind ?? ''] ?? 'випуск'));
