 ...
 *     б жест показу табла (HOTKEYS-v9 § 2.1).
 *  6. Накладка, що забрала клавіатуру собі (`captureKeyboard`). Поле вводу — не
 *     єдине місце, де літера означає не те, що зазвичай: піаніно в підвалі читає
 *     `KeyG`, `KeyH`, `KeyJ` і `KeyR` як ноти, а полем вводу не є, тож пункт 2
 *     його не покриває. Без цього обмеження сім однакових нот поспіль вели зі
 *     сторінки в «Галактику випускників» посеред гри.
 *
...
import { isKeyboardCaptured, isPlainKey, isTypingTarget } from './keyboard';
...
export function createKeySequence(options: KeySequenceOptions): KeySequence {
	...
	return {
		...
		handle(stroke: KeyStroke): void {
			/*
			 * Автоповтор, набір тексту, комбінації й зайнята накладкою клавіатура не
			 * рахуються — але й НЕ скидають лічильник: натиснути потрібну клавішу,
			 * зачепивши поле вводу або втримавши `Ctrl`, і втратити вже набране було б
			 * поведінкою, якої ніхто не пояснить.
			 */
