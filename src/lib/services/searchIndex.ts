...
export async function newsEntries(lang: 'uk' | 'en'): Promise<SearchEntry[]> {
	...
	try {
		...
		for (const article of articles) {
			...
			entries.push({
				id: `news:${article.id ?? article.slug ?? tr.title}`,
				title: tr.title,
