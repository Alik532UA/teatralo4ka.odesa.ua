...
/**
 * BETA-CHECKLIST-v9 § 3.1 (позначка несе версію) і § 6 (звіт).
 *
...
describe('позначки чеклиста', () => {
	...
	it('зіпсований запис відкидається поштучно, а не разом з усіма', () => {
		...
		//
		// Ідентифікатори тут СПРАВЖНІ, з чеклиста: із § 8.6 позначка невідомого
		// пункта теж відкидається, тож вигадані `good_1`/`bad_1` перевіряли б
		// зовсім не те, що написано в назві тесту.
		const other = BETA_CHECKS[1].id;
		storage.set(
			KEY,
			JSON.stringify({ [SOME_ID]: { vote: 'ok', version: '1.0.0' }, [other]: { vote: 'maybe' } })
		);
		expect(loadMarks()).toEqual({ [SOME_ID]: { vote: 'ok', version: '1.0.0' } });
	});

	/**
	 * § 8.6 `BETA-MARKS-UNTRUSTED`. Форму такий запис має правильну, тож до 9.3
	 * він проходив і рахувався в поступі: пункт прибрали зі списку, а число
	 * лишилося — сторінка показувала «36 / 34», і виправити це зсередини не було
	 * як, бо знімати позначку вже нема на чому.
	 */
	it('позначка пункта, якого в чеклисті вже немає, не рахується (§ 8.6)', () => {
		storage.set(
			KEY,
			JSON.stringify({
				[SOME_ID]: { vote: 'ok', version: '1.0.0' },
				vanished_99: { vote: 'ok', version: '1.0.0' }
			})
		);

		const marks = loadMarks();
		expect(marks[SOME_ID], 'перевірка мертва: справжня позначка теж зникла').toBeDefined();
		expect(Object.keys(marks), 'позначка видаленого пункта вижила').toEqual([SOME_ID]);
		expect(countFresh(marks, '1.0.0'), 'поступ рахує пункт, якого немає в списку').toBe(1);
	});
