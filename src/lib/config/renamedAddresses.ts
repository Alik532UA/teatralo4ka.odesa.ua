...
/**
 * Адреса випускника → сторінка ПРАЦІВНИКА. Єдина мапа, що веде через розділ.
 *
 * Решта мап переставляють адресу всередині свого розділу: випускник лишається
 * випускником, майстер — майстром. Тут інше: людину помилково завели не в тому
 * реєстрі, і виправлення означає перехід із «Галактики випускників» у «Дорослі
 * мешканці». Ціль тому й пишеться повним хвостом `residents/adults/…`.
 *
 * Окремою мапою, а не рядком у `RENAMED_GRADUATE_ADDRESSES`: та мапа обіцяє, що
 * ціль — чинна адреса ВИПУСКНИКА, і на цьому стоїть перевірка «ціль існує».
 * Змішати їх означало б зламати обіцянку заради одного запису.
 */
export const GRADUATE_MOVED_TO_ADULT: Record<string, string> = {
	// 2026-09-08: Анастасія Ніколаєва. Автор назвав її серед учасниць поїздки на
	// Art Like Fest 2026, і 7 вересня вона з'явилася в реєстрі випускників як
	// учениця — при тому, що в реєстрі майстрів уже роками стоїть викладачка
	// вокалу з тим самим іменем. Наступного дня автор відповів: людина ОДНА, і
	// вона викладачка. Запис учениці прибрано.
	//
	// Проста прибирання тут замало: коміт із помилкою поїхав у `main`, тобто
	// адреса встигла побувати в `sitemap.xml`. Мовчазна 404 читалася б як
	// «людину видалили» — а вона нікуди не діла́ся, просто живе в іншому
	// розділі, і фестиваль у неї той самий.
	'anastasiia-nikolaieva': 'anastasiia-nikolaieva'
};
...
export const RENAMED_PATHS: readonly (readonly [старий: string, новий: string])[] = [
	...
	...Object.entries(GRADUATE_MOVED_TO_ADULT).map(
		([с, н]) => [`/projects/galaxy-graduates/${с}`, `residents/adults/${н}`] as const
	)
];
