...
describe('новини в коді', () => {
	...
	/**
	 * Адреса новини стоїть у переліку пререндера — інакше англійської сторінки
	 * НЕ БУДЕ.
	 *
	 * `entries()` у `news/[id]/+page.ts` дає лише українську адресу; англійська
	 * народжується з `PUBLIC_ENTRIES` у `svelte.config.js`, бо мовний префікс тут
	 * робить хук `reroute`, а не маршрут. Це записано там-таки в коментарі — і
	 * все одно повторилося: 5 вересня 2026 новину додали без рядка, і в `build/`
	 * була `news/30th-season-opened-2026/`, а `en/news/…` не було.
	 *
	 * Перевірка тут, а не в `generate-sitemap`, бо той звіряє В ОДИН БІК: що
	 * названа адреса існує в збірці. Забутий рядок він не бачить за визначенням.
	 */
	it('кожна новина з коду названа в переліку пререндера', () => {
		const конфіг = readFileSync('svelte.config.js', 'utf8');
		const bad = CODE_NEWS.filter((item) => !конфіг.includes(`'/news/${item.id}'`)).map(
			(item) => item.id
		);
		expect(
			bad,
			'без рядка в `PUBLIC_ENTRIES` англійська сторінка новини не збереться, ' +
				`і помітити це можна лише за числами в мапі сайту:\n  ${bad
					.map((id) => `'/news/${id}',`)
					.join('\n  ')}`
		).toEqual([]);
	});
