 ...
 * `firebase/firestore.rules` і виконуються на боці Firebase. Відколи їх
 * викладає CI (job `publish-rules` у `deploy.yml`, 2026-09-21), розходження
 * між файлом і базою принаймні не додається до розходження між кодом і
 * файлом — доти ж правила їхали руками, і обидва розходження складалися.
 *
 ...
 * немає. CI їх викладає з 2026-09-21 (`publish-rules`), але звірка файлу з
 * базою нічого не каже про звірку файлу з КОДОМ — це два різні розходження.
 *
...
/**
 * ПРАВИЛА ДОЇЖДЖАЮТЬ ДО БАЗИ — і це перевіряється, а не мається на увазі.
 *
 * Перевірки вище доводять, що ФАЙЛ правильний. Що приймає `promo-web-hub`, з
 * них не випливає нічого: правила виконуються на боці Firebase, і доти їх
 * викладала людина командою `npm run deploy:rules`. Ризик тут вищий за
 * середній, бо проєкт Firebase один на всі сайти шкіл — розходження означає
 * не «одна сторінка зламалась», а «межа доступу не та, яку читали в рев'ю»
 * (CLOUD-DATABASE-v9 § 2.3, `CDB-RULES-IN-REPO`, `CDB-RULES-READBACK`).
 */
describe('правила доїжджають до бази', () => {
	...
	it('індекси їдуть тим самим кроком', () => {
		expect(
			workflow,
			'емулятор індексів не перевіряє взагалі, а Firestore без індексу відмовляє лише ' +
				'в продакшні — тобто запит, зелений локально, падає в людей'
	...
	it('викладання йде ПЕРЕД збіркою', () => {
		expect(
			workflow,
			'build-and-deploy більше не чекає на publish-rules: нові правила сумісні зі старим ' +
				'кодом, старі з новим — ні, тож порядок тут не стильовий'
		).toContain('needs: publish-rules');
	});

	/*
	 * Скрипт, який лежить у репозиторії, але якого ніхто не викликає, — це не
	 * перевірка, а спогад про неї. Саме так сталося в сусідньому `Slovko`:
	 * файл був, крок у воркфлоу забули, і зелений прогін нічого не звіряв.
	 */
	it('після викладення правила читаються з бази назад', () => {
		expect(
			workflow,
			'скрипт звірки є, але воркфлоу його не викликає — тоді «правила викладено» ' +
				'знову означає лише «команда не впала»'
		).toContain('scripts/verify-deployed-rules.mjs');

		const script = readFileSync('scripts/verify-deployed-rules.mjs', 'utf8');
		expect(
			script,
			'звірка байт-у-байт червонітиме на правильних правилах: відступи віддає Firebase ' +
				'у своєму форматуванні'
		).toMatch(/replace\(\/\\s\+\/g, ' '\)/);
		expect(
			script,
			'проєкт мусить братися з того самого джерела, що й у застосунку, інакше перевірка ' +
				'може сумлінно звіряти правила чужої бази'
		).toMatch(/CONFIG_SOURCE\s*=\s*'src\/lib\/firebase\/config\.ts'/);
	});
});
