...
/**
 * Інваріанти чеклиста бета-тестування (BETA-CHECKLIST-v9 § 5).
 *
...
/**
 * Сторінки, ЯКІ ІСНУЮТЬ, — із файлової системи, а не лише зі списку в конфізі.
 *
 * `prerender.entries` вписують руками, і туди потрапляє не все: розділи
 * галактики пререндеряться власними `entries()` у кожному маршруті, тож у
 * конфізі їх немає. Через це перевірка «кожен маршрут заявлений вкладкою»
 * роками не бачила ВІСЬМОХ сторінок — вистави, групи, поїздки, числа, заклади
 * освіти, театри, фахівці й сторінка самого випускника. Тобто мовчала не тому,
 * що все покрито, а тому, що не знала про них.
 *
 * Адреси з `[…]` пропускаються: одна така тека — це сотні сторінок, і заявити
 * їх маршрутом не можна. Їх покривають пункти з КОНКРЕТНОЮ адресою, яка
 * приходить зі списку в конфізі.
 */
const FS_ROUTES: string[] = walk('src/routes')
...
describe('чеклист бета-тестування: дані', () => {
	...
	it('кожен маршрут проєкту заявлений рівно однією вкладкою (§ 5.1)', () => {
		...
		const routes = [...new Set([...PRERENDER_ENTRIES, ...FS_ROUTES])]
