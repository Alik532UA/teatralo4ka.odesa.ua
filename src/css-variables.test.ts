 ...
 * проєкті (UI-UX-v9 § 1.6). Він не дає ні помилки збірки, ні попередження
...
/**
 * `light-dark()` з НЕколірним аргументом (UI-UX-v9 § 1.5.1.3,
 ...
 *
 * ЦЕЙ ПРОЄКТ НА VITE 7, І ЦЕ ВАЖЛИВО — але рятує НЕ ТЕ, що здавалося.
 *
 * Спершу тут було написано, що справа в `css.transformer`. Канон це виправив у
 * v9: `css.transformer` ні до чого — у `VetCrewGames` він типовий (`postcss`), а
 * знижує `light-dark()` МІНІМІЗАТОР. `build.cssMinify` для клієнтської збірки
 * типово `true`, і Vite 8 маршрутизує це в Lightning CSS, який зводить виклик до
 * пари підстановок `--lightningcss-light` / `--lightningcss-dark` для будь-якого
 * типу значення. Vite 7 віддає мінімізацію esbuild, який не знижує нічого.
 *
 * Заміряно у власному `build/` від 26.08: там лежить рівно
 ...
 * Тобто працездатність там тримає НЕДЕКЛАРОВАНИЙ ДЕФОЛТ: `cssMinify: 'esbuild'`,
 * `cssMinify: false` або наступний мажор Vite знімають єдиний бар'єр без жодного
 * червоного. Саме тому гейт джерела нижче обовʼязковий на будь-якій версії.
 *
 * Імена
 ...
 *
 * Зворотний експеримент (AI-AGENT-PITFALLS-v9 § 1.1): вписати в
...
describe('light-dark() приймає лише колір (UI-UX-v9 § 1.5.1.3)', () => {
