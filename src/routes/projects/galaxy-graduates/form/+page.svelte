<script lang="ts">
	 ...
	 * `/projects/galaxy-graduates/?form=open`. Прев'ю в месенджері брало опис
	...
	const target = $derived(
		`${localizedPath('/projects/galaxy-graduates/', lang)}?form=open`
	);
...
<svelte:head>
	<title>{$t('seo.pages.galaxyForm.title')} | {$t('hero.title')}</title>
</svelte:head>
...
-->
<main class="hop" data-testid="graduate-form-page-section">
	<a class="hop__link" href={target} data-testid="graduate-form-page-galaxy-link">
		<span>{$t('seo.pages.galaxyForm.title')}</span>
