<script lang="ts">
	 ...
	 *
	 * Черга складається один раз — у `toggleSlideshow`, — і доти нічого її не
	 * перескладало. Тобто людина обирала «З закладом освіти або театром», а
	 * бачила далі тих самих, кого набрав попередній фільтр: напис обіцяв одне,
	 * показ робив інше. Автор це й побачив.
	 *
	 * Перший слайд нової черги показується одразу, не чекаючи таймера: інакше
	 * до кількох секунд на екрані лишалася б людина, яка під новий фільтр уже
	 * не підходить.
	 */
	let попереднійФільтр = slideshow.filter;
	$effect(() => {
		const фільтр = slideshow.filter;
		if (!slideshow.active) {
			попереднійФільтр = фільтр;
			return;
		}
		if (фільтр === попереднійФільтр) return;
		попереднійФільтр = фільтр;
		/* `untrack` — щоб запис у чергу й крок не зробив цей ефект залежним від
		   власного результату. */
		untrack(() => {
			черга = [...показ].sort(() => Math.random() - 0.5);
			крок = 0;
			...
			replaceState(profileHref(черга[0]), { graduateAddress: graduateAddress(черга[0]) });
		});
	});

	/**
	 * АНКЕТА НАСТУПНОГО ЗАВАНТАЖУЄТЬСЯ ЗАЗДАЛЕГІДЬ.
	 *
	 * Картка малюється у два прийоми: спершу те, що є в індексі (ім'я, рік,
	 * портрет), а вистави, біографія й галерея приходять окремим запитом. На
	 * слайдшоу це видно як стрибок: основа стала, а за мить під нею
	 * розгорнувся вміст і зсунув усе вниз.
	 *
	 * Тому поки людина дивиться поточного, ми вже тягнемо анкету наступного:
	 * до моменту підміни вона лежить у кеші, і картка малюється одразу цілою.
	 * Запит один і дешевий — найбільша анкета 9 КБ, — а `ensureGraduateProfile`
	 * сам мовчить, якщо вона вже є.
	 */
	$effect(() => {
