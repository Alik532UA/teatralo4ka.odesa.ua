<script lang="ts">
	...
	async function handleLogin() {
		...
		try {
			...
			error = 'Помилка входу. Перевірте пошту та пароль.';
			console.error(e);
		} finally {
			loading = false;
		}
...
<section class="admin-login container" style="padding: 160px 24px; max-width: 400px;">
	<div style="background: white; padding: 2.5rem; border-radius: 40px; box-shadow: 0 20px 50px rgba(0,95,174,0.1);">
		<h1 style="font-family: var(--font-heading); color: var(--color-deep-ocean); margin-bottom: 2rem; text-align: center;">Вхід в Адмін-панель</h1>
