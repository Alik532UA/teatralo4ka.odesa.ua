<script lang="ts">
	...
	let contentType = $state<ContentType>('article');
	...
	$effect(() => {
		if (!authService.loading) {
			if (!authService.isAuthenticated) {
				...
				const perms = profile?.projects?.[PROJECT_ID]?.permissions;
				const canEditArticles = isSuperAdmin || perms?.canEditArticles === true;
				const canEditPages = isSuperAdmin || perms?.canEditPages === true;
...
{#if loading}
	<section class="container" style="padding: 120px 24px;">
		<div style="display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 400px; gap: 1rem;">
			...
			<p>{get(t)('admin.articles.loading')}</p>
		</div>
	</section>
{:else if articleData}
	<ArticleForm
