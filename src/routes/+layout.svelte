...
<style>
	...
	/*
	 * Копія в розгоні — НЕРУХОМИЙ знімок смуги, а не друга жива сторінка.
	 *
	 * Без цього на шві було видно постійну появу героя: копію перебудовують
	 * щоразу, коли жива смуга змінилася, а кожна вставка запускає CSS-анімації
	 * в ній З НУЛЯ. Читач, що під'їжджає до шва, бачив герой, який проявляється
	 * знову й знову — тобто саме те, чого на справжній сторінці не буває.
	 *
	 * Гаситься не `animation: none`, а ТРИВАЛІСТЬ, і це не дрібниця. Частина
	 * появ на сайті починається з `opacity: 0` і стає видимою власне анімацією
	 * (`fadeInUp` — див. `e2e/ready.ts`); `none` лишило б такі блоки прозорими
	 * назавжди, тобто копія вийшла б порожньою там, де смуга не порожня.
	 * Нульова тривалість разом із `forwards` дає протилежне: анімація миттєво
	 * стає у свій КІНЦЕВИЙ кадр. Нескінченні пульсації так само завмирають.
	 */
	.endless-runway :global(*),
	.endless-runway :global(*::before),
	.endless-runway :global(*::after) {
