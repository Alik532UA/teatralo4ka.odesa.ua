...
<style>
	...
	.gallery-bento__subtitle {
		font-size: 1.2rem;
		/*
		 * ПРИГЛУШЕНИЙ КОЛІР, А НЕ `opacity` НА ОСНОВНОМУ.
		 *
		 * Тут стояло `color: var(--color-body-text); opacity: 0.7`, і Lighthouse
		 * давав `color-contrast` саме на цьому рядку: прозорість змішує текст із
		 * тлом, тобто знижує контраст, лишаючи в коді колір, який його проходить.
		 * У звіті це виглядає як помилка кольору, хоч колір правильний.
		 *
		 * `--color-muted-text` для приглушеного тексту й призначений: він уже
		 * підібраний під тло, і його контраст не залежить від того, що лежить
		 * позаду.
		 */
