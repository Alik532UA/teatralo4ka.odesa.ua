 ...
 *
 * ACCESSIBILITY-v9 називає модалку без пастки й без повернення фокуса
...
describe('модалки', () => {
	...
	it('кожен aria-modal="true" має {@attach focusTrap(...)} на тому самому елементі', () => {
		...
		expect(
			missing,
			`модалки з aria-modal="true" без пастки фокуса (ACCESSIBILITY-v9, HIGH):\n${missing.join('\n')}`
		).toEqual([]);
	});
	 ...
	 *
	 * Зворотний експеримент (AI-AGENT-PITFALLS-v9 § 1.1): прибрати `aria-modal`
	...
	it('перевірка справді знаходить модалки, а не порожній набір', () => {
		...
		// тест-заглушка, лише непомітна (AI-AGENT-PITFALLS-v9 § 1).
