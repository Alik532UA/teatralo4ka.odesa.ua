...
const КОРІНЬ = 'src';
/**
 * Шари, чиї експорти хтось мусить кликати ЗЗОВНІ.
 *
 * `utils/` додано разом із `installImageReveal`: там уже лежали три `install*`
 * (`installTrail`, `installViewTransitions`, `installMailtoToast`), і жоден із
 * них не був під цим інваріантом — тобто найдешевший спосіб зробити механізм
 * мертвим (написати й не покликати з макета) не стеріг ніхто.
 */
const ШАРИ = ['src/lib/services/', 'src/lib/utils/'];
const уШарі = (файл: string) => ШАРИ.some((шар) => файл.startsWith(шар));
...
describe('джерела Svelte (SVELTE-CORE-v9)', () => {
	...
	it('кожну підписку сервісного шару хтось кличе поза ним', () => {
		const ПІДПИСКА = /^(subscribe|init|watch|listen|start|install)[A-Z0-9_]?/;

		/*
		 * РЯДКИ ІМПОРТУ ВІДСІКАЮТЬСЯ, і це виправлення дефекту самого гейта.
		 *
		 * Перша редакція вважала за виклик будь-яку згадку імені — а імпорт це
		 * теж згадка. Тобто модуль, який чесно імпортували в макет і забули
		 * покликати, гейт пропускав, хоч рівно це він і мусить ловити.
		 * Знайшлося зворотним експериментом: прибрав `installImageReveal();` із
		 * `+layout.svelte`, лишивши імпорт, — і все лишилося зеленим.
		 *
		 * Гола згадка імені у переліку лишається навмисно: підписку буває
		 * передано посиланням (`afterNavigate(initX)`), і це так само підключення.
		 * Відсікається саме імпорт, а не будь-що без дужок.
		 */
		const ззовні = джерела
			.filter(({ файл }) => !уШарі(файл))
			.map(({ текст }) => текст.replace(/import\s[\s\S]*?from\s*['"][^'"]+['"]\s*;?/g, ' '))
			.join('\n');
	...
	it('модуль зі слухачем має парне зняття', () => {
		...
		const ВИНЯТКИ = new Map([
			...
			[
				'src/lib/utils/installImageReveal.ts',
				'`MutationObserver` на документ мусить переживати кожну навігацію: ' +
					'він для того й стоїть, щоб накрити зображення, які додадуть пізніше. ' +
					'Кличеться один раз із кореневого макета, чиє розмонтування означає ' +
					'вихід із сайту, — тобто зняття не має ні виконавця, ні миті'
			]
		]);
