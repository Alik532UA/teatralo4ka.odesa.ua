...
describe('борг ESLint — число, що лише спадає (CODE-QUALITY-v9 § 6.4.3)', () => {
	...
	beforeAll(async () => {
		...
		//
		// 300 с замість 120: запас перестав бути запасом. Заміряно 2026-09-21
		// на НЕЗАВАНТАЖЕНІЙ машині, без решти прогону: 659 файлів, 140.1 с.
		// Тобто поріг 120 с не витримував уже й сам по собі, а в повному
		// прогоні цей хук ділить машину зі 145 паралельними форками. Число
		// вибране як подвоєний вимір, і воно піде вниз, щойно піде вниз сам
		// прохід.
	...
	it('перевірка жива: lint пройшов по джерелах проєкту', () => {
		 ...
		 *
		 * 344 — заміряно прогоном на момент коміту. Межа вдвічі нижча: не
