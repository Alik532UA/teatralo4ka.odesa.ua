...
const VERDICT: Record<string, { axis: Axis; why: string }> = {
	'PlayCastSection.svelte': {
		axis: 'window',
		why:
			'секція займає всю ширину сторінки вистави й ніде більше не стоїть — ' +
			'місце, яке їй дісталося, ДОРІВНЮЄ вікну мінус поля сторінки. Сітка ' +
			'карток міняється на 480px разом із рештою секцій тієї ж сторінки ' +
			'(майстри, репертуар), і рахувати її від контейнера означало б, що ' +
			'сусідні секції ламаються в одному місці, а ця — в іншому'
	},
	'ContentCard.svelte': {
		axis: 'container',
		why:
	...
	'GalaxyStageControls.svelte': {
		axis: 'window',
		why:
			'п’ять кнопок керування сценою стоять смугою на всю ширину ЕКРАНА і на ' +
			'вузькому ховаються за один значок; місця, яке дісталося, у них немає — ' +
			'контейнером їм служить сама сцена `position: fixed; inset: 0`, тобто те ' +
			'саме вікно (заміряно: рядок потребує ~620px, на 375 переносився у три)'
	},
	'ui/FilterDropdown.svelte': {
		axis: 'window',
		why:
			'кнопка фільтра тягнеться на всю ширину не тому, що їй мало МІСЦЯ, а ' +
			'тому, що на вузькому ЕКРАНІ рядок фільтрів перестає вміщати їх поруч ' +
			'і переноситься; те саме правило доти стояло в `GraduateRosterFilters` ' +
			'і переїхало сюди разом із кнопкою'
	},
	'ContactMenuBody.svelte': {
		axis: 'window',
		why:
			'вміст віконця «напиши мені»: розмір йому задає ПРОП від віконця, а ' +
			'єдиний запит тут — про те, що на вузькому екрані смужка не ' +
			'вміщається у ВІКНО (заміряно 343 пікселі проти 375) і віконце ' +
			'складається в прямокутник, де щільні рядки треба розсунути; місця, ' +
			'яке дісталося, у нього немає — його контейнер кругла кнопка'
	},
	'EditContactButton.svelte': {
		axis: 'window',
		why:
			'меню висить `position: absolute` біля круглої кнопки 36 пікселів, тож ' +
			'МІСЦЯ, яке дісталося, тут немає взагалі — контейнерний запит міряв би ' +
			'кнопку. Обмежує його рівно ВІКНО: смужка `--card` виїжджає ліворуч і на ' +
			'375px виходила за край на 99 пікселів (заміряно), тому на 560px вона ' +
			'падає вниз прямокутним вікном'
	},
