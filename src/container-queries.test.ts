...
const VERDICT: Record<string, { axis: Axis; why: string }> = {
	'GalleryThumbRail.svelte': {
		axis: 'window',
		why:
			"стрічка прев'ю притулена до краю ВІКНА всередині накладки на весь " +
			'екран — місця їй не дає ніхто, вона сама частина накладки. ' +
			'Контейнерного запиту тут нема чого міряти: контейнера в неї немає, ' +
			'а межа 768 px описує саме вікно — на телефоні кожен піксель стрічки ' +
			'забирається просто із самої світлини, тож відступи там менші'
	},
	...
	'galaxy/GalaxyRegistryHeader.svelte': {
		axis: 'window',
		why:
			'шапка реєстру займає всю ширину сторінки в усіх реєстрах галактики — ' +
			'місце, яке їй дісталося, і є вікно мінус поля сторінки. На 640 px ' +
			'пошук і додатковий перемикач чи область показу складаються в колонку ' +
			'й скидають вирівнювання праворуч, узгоджено з рештою шапок сайту'
	},
	...
	'planet/PlanetSplit.svelte': {
		axis: 'window',
		why:
			'дві колонки — планета й перелік імен — стоять на всю ширину сторінки, ' +
			'тож місце, яке їм дісталося, і є вікно мінус поля. Межа 860 px описує ' +
			'саме вікно: вужче за неї дві колонки поруч не мають сенсу, бо кожна ' +
			'стискається до непридатного. Контейнерного запиту тут мало б бути ' +
			'мало — сам компонент і є верхній рівень сторінки. Контейнер у ньому ' +
			'все ж є, але інший і на іншому рівні: `.planet` міряє САМА СЕБЕ, щоб ' +
			'обличчя рахувалися в `cqw` від кулі, а не від вікна'
	},
	'PhotoBentoGallery.svelte': {
		axis: 'window',
		why:
			'галерея стоїть на всю ширину сторінки в обох місцях, де є, — «Про школу» ' +
			'і сторінка новини, — тож місце, яке їй дісталося, дорівнює вікну мінус ' +
			'поля. Розмірні запити тут міняють КІЛЬКІСТЬ КОЛОНОК (4 → 2 → 1), а її ' +
			'контейнерним запитом порахувати не можна: колонок на момент виміру ще ' +
			'не існує (§ 7A, той самий висновок, що в `ContentWidget`). Стилі ' +
			'перенесені зі сторінки «Про школу» без змін, і ця межа була в них від ' +
			'початку — витяг не привід міняти поведінку, а привід її записати'
	},
